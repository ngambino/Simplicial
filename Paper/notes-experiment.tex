...
A map $f \co \Psi \rightarrow \Phi$ in $\SSet$ is a \myemph{strong $k$-oriented homotopy equivalence} if there are maps $H$ and $H_X$ which exhibit $f$ as a retract of $\delta^k \times ' f$ as follows:
...
\item One can simply take $p \co \Phi.A \to \Phi$ to be $\Pi_f(q) \co \Phi.\Pi_f(B) \to \Phi$. Indeed, it is a trivial fibration by part~$(i)$ and the square is a pullback by part $(ii)$. The final remark about the cofibrancy of 
$\Phi.A$ follows from $(iii)$. \qedhere
\end{enumerate}
\end{proof}
