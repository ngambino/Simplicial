...
the category of simplicial sets, \ie the presheaf category $[\Delta^{\op}, \Set]$ by $\SSet$. 
...
Note that cofibrations between cofibrant objects are the levelwise complemented monomorphisms. 
Cofibrant simplicial sets are of particular importance for our development because of their decidability properties, which can be used to establish counterparts of classical results valid for all simplicial sets. An example is the Eilenberg-Zilber lemma~\cite{henry2018wms}, asserting that in a cofibrant simplicial set~$X$, any cell $x \in X$ can be written uniquely as $p^*(y)$, where $y$ is a non-degenerate cell of $X$ and $p$ is a degeneracy. It is clear that, assuming the law of excluded middle, every monomorphism is a cofibration
and thus our notion of a cofibration coincides with the classical one. 
...
\item A simplicial set $A$ will be said to be \myemph{fibrant} if the unique map $A \to 1$ is a fibration.
\end{enumerate}
\end{definition}
...
Having defined cofibrations and fibrations, one could try to establish the existence of a Quillen model
structure by defining a trivial fibration to be a map with the right lifting property with respect to all cofibrations,
a trivial cofibration to be a map with the left lifting property with respect to all fibrations, and a weak equivalence to be a map that can be factored as a trivial cofibration followed by a trivial fibration. Indeed, 
...
For this, let us define an \emph{acyclic fibration} to be a map with the right lifting property
with respect to every cofibration with cofibrant domain and an \myemph{acyclic cofibration} to be 
a map with left lifting property with respect to all fibrations with fibrant codomain.
Note that, being defined by weak orthogonality conditions, the classes of acyclic fibrations
and acyclic cofibrations have the usual closure properties. We also say that a simplicial set is \myemph{bifibrant} if it is both fibrant and cofibrant. 

\bigskip

As shown in~\cite{henry2018wms}, cofibrations and fibrations as defined above form a  a weak model structure on the category $\SSet$, in the sense that the following conditions hold.
...
We refer the reader to~\cite{henry2018wms} for the details of the proof and limit ourselves to a few comments to illustrate in what ways the weak model structure on $\SSet$ relates to the Quillen model structure. 
For this, let us define the set of boundary inclusions $\cal{I}$ and the set of horn inclusions $\cal{J}$ by letting
\[
...
\begin{equation}
\label{equ:wfs}
\big( \mathsf{Sat}(\cal{I}) \, , \cal{I}^\pitchfork \big) \, , \qquad 
\big( \mathsf{Sat}(\cal{J}) \, , \cal{J}^\pitchfork \big) \, .
\end{equation}
As shown in~\cite[Proposition 5.1.7]{henry2018wms} a map is a cofibration if and only if it is in $\mathsf{Sat}(\cal{I})$. Furthermore, since the domains of the boundary inclusions are cofibrant simplicial sets, acyclic fibrations have the right lifting property with respect to all cofibrations, not just those with cofibrant domain, and therefore they are exactly the maps in $\cal{I}^\pitchfork$. Thus, the first weak factorisation system in~\eqref{equ:wfs} has cofibrations as left maps and acyclic fibrations as right maps. It is clear how this 
weak factorisation system is the counterpart of the classical weak factorisation system of monomorphisms
and trivial Kan fibrations. 

For the second weak factorisation system, the situation is slightly more subtle. In order to discuss it, it is
convenient to introduce some additional terminology and refer to the maps in~$\mathsf{Sat}(\cal{J})$ as \myemph{anodyne extensions}. It follows easily from the definitions that every anodyne extension is an acyclic cofibration. 
