...
\section{Product types and substitution} 

\noten{This section should define the outline of the interpretation and some basic results. Some material
should be moved here from the `Outline of the interpretation' section.}
...
\section{Function types and identity types}


\subsection*{The pushout-product property}
...
\end{itemize}
then $\langle f , p \rangle$ is a trivial fibration. 

\bigskip

The following lemma gives a list of special cases of these observations that we will use later.
...
An immediate consequence of~\cref{lemma:dualPushoutProductSpecialCases} is the following 
proposition, which provides a first step towards the interpretation of function types. 

\begin{proposition} \label{thm:exponentials}
 Let $X$ be cofibrant and $A$ be fibrant.  Then $A^X$ is fibrant.
\end{proposition}

\begin{proof} Apply part~(i) of \cref{lemma:dualPushoutProductSpecialCases} with $Y = 0$.
\end{proof} 


\noten{Discussion of the cofibrant replacement, validity of the $\beta$-rule judgementally and of the
$\eta$-rule propositionally.}
...
Let $A$ be a bifibrant simplicial set. We wish to the path object of $A$ as the exponential $A^{\Delta[1]}$,
which is fibrant by~\cref{thm:exponentials}.


\begin{lemma} Assume that $A \in \SSet$ is fibrant. Then:
\begin{enumerate}[(i)] 
\item the map $A^{\Delta[1]} \rightarrow A \times A$ is a fibration.
\item the composite of $A^{\Delta[1]} \rightarrow A \times A$ with either projection is a trivial fibration,
\item the map $A \rightarrow A^{\Delta[1]}$ induced by $\Delta[1] \rightarrow \Delta[0]$ is a weak equivalence.
\end{enumerate}
\end{lemma} 

\begin{proof}
For part (i), apply part~(i) of \cref{lemma:dualPushoutProductSpecialCases} to the cofibration $\partial \Delta[1]  \hookrightarrow \Delta[1]$. For part~(ii), apply part~(ii) of \cref{lemma:dualPushoutProductSpecialCases} to the anodyne maps $\Lambda^k[1]  \hookrightarrow \Delta[1]$. Part~(iii) follows from the $2$-out-of-$3$ property for weak equivalence between fibrant objects applied to $A \rightarrow A^{\Delta[1]} \rightarrow A$. Indeed, the
composite is the identity and the second factor has just been proved to be a trivial fibration.
\end{proof}


In order to have a path object, we would need $A^{\Delta[1]}$ to be also cofibrant and that the map $A \rightarrow A^{\Delta[1]}$ is a trivial cofibration. In a general setting, these properties are not to be expected
...
Let $A$ be a cofibrant simplicial set. Then $A^{\Delta[1]}$ is cofibrant, and the map $A \rightarrow A^{\Delta[1]}$ is a cofibration.
\end{proposition}
...
More generally, we need to define the identity type of a dependent type, \ie for a fibration $p \co A \twoheadrightarrow X$ between bifibrant objects. One defines $\Id_A$ as the pullback:
...
\item the maps $s \, , t \co \Id_A \rightarrow A \times_{X} A \rightarrow A$ obtained by composing $(s,t)$ with any of the two projections is a trivial fibration,
...
\] 
of a map which has been proved to be a levelwise complemented inclusion in~\cref{proposition:PathObjectCofibrant}, hence for any cell of $\Id_A$ one can decide if it is in $A$ or not by considering it as a cell of $A^{\Delta[1]}$. As $A$ and $\Id_A$ are cofibrant, this shows that $A \rightarrow \Id_A$ is a cofibration. The $2$-out-of-$3$ property applied to $A \rightarrow \Id_A \rightarrow A$ show that  $A \rightarrow \Id_A$ is moreover a weak equivalence, hence an acyclic cofibration.
\end{proof}
