...
\section{A universal fibration with cofibrant base}
...
\begin{lemma} \label{thm:fibrancy-of-u} The simplicial set $\U$ is fibrant. 
\end{lemma}
...
\begin{theorem} \label{thm:fibrancy-of-uc} The simplicial set $\U$ is fibrant. 
\end{theorem}

\begin{proof} Immediate from ~\cref{thm:fibrancy-of-u}, since $\tau \co \U_c \to \U$ is a trivial fibration.
\end{proof}


We now wish to define  what it means for a small fibration between cofibrant objects  be univalent. For this, we fix such a fibration $p \co A \to X$ and construct an object $\Weq(p) \to X \times X$ that represents weak
equivalences between fibers of $p$, in the sense that maps $\sigma \co Y \to \Weq(p)$ in $\SSet_{/X \times X}$ are in bijective correspondence with maps $x_1 \, , x_2 \co Y \to X$ and weak equivalences $w \co A[x_1]
\to A[x_2]$ in $\SSet_{/Y}$. The required object $\Weq(p)$ can be construted as the pullback
\[
\xymatrix{
...
has the required universal property is an easy calculation, which we leave to the readers.




\begin{proposition}  \label{equ:characterisations-of-univalence} Let $p \co A \to X$ be a small fibration between cofibrant objects. Then following equivent conditions are equivalent.
\begin{enumerate}[(i)] 
\item The map $i \co X \to \Weq(p)$ is an equivalence. 
\item The map $s \co \Weq(p) \to X$ is a trivial fibration. 
\item The map $t \co \Weq(p) \to X$ is a trivial fibration. 
\item The map $j \co \Id(X) \to \Weq(p)$ fitting in the diagram
\[
\xymatrix@C=1.5cm{
...
\begin{proof} Immediate consequences of the  3-for-2 property.
\end{proof}



\begin{theorem} \label{thm:univalence-of-uc} The fibration $\pi_c \co \UU_c \to \U_c$ is univalent.
\end{theorem}

\begin{proof}  We prove that $t \co \Weq(\pi_c) \to \U_c$ is a trivial fibration. For this, let us
