...
the pushout product
\[
i \hattimes \kcyl \big( \Lambda^k[n] \times \Delta[1] \big) \cup (\Delta[n] \times {k}) \to \Delta[n] \times \Delta[1] \, .
\]
\end{lemma} 
...
set $X$ to be \emph{small} if $X_n$ is a small set for every $[n] \in \Delta$. 


\begin{definition} We say that a map $f \co Y \to X$ in $\SSet$ is small if for every $x \co \Delta[n] 
\to X$ the simplicial set $Y_x$ is small.
\end{definition} 

By the results in~\cite{HofmannM:universe} for arbitrary presheaf categories, small maps in
...
\[
\xymatrix{
\UU_c \ar[d]_{\pi_c} \ar[r] & \UU \ar[d]^{\pi}  \\
\U_c \ar[r]_p & \U}
\]
...
\begin{proof} Part~(i) follows directly from part~(i) of~\cref{thm:universe-u}. For part~(ii), 
let $p \co Y \to X$ be a small Kan fibration with $X$ cofibrant. Since $p$ is a
