...
the so-called Frobenius property, \ie that pullback along trivial cofibrations preserves fibrations. 
As is well known, by adjointness this implies that, for a fibration $p \co A \rightarrow X$, the dependent product functor $\Pi_p \co \SSet_{/A} \to \SSet_{/X}$ preserves fibrations. The proof of the Frobenius property follows essentially~\cite{gambino2017frobenius}. In order to combine our development with that argument, however, we need the following preliminary proposition, in which we use the pushout product $\kcyl \hattimes f$ of an endpoint inclusion $\kcyl \co \braces{ k } \to \Delta[1]$ and a map~$f \co Y \to X$, which is defined as the unique dotted arrow  in the diagram:
\[
\xymatrix{
...
\item Any small map $p \co A \rightarrow X$ is a pullback of $\rho \co \VV \rightarrow \V$. The corresponding map $a \co X \rightarrow \V$ factors in $\U \subseteq \V$ if and only if for every $\Delta[n] \rightarrow X$ the pullback of $A$, $E \rightarrow \Delta[n]$ is a small fibration with $E$ cofibrant. So if $A$ is cofibrant then $E$ is cofibrant because of~\cref{lem:cofibrant_fiber_product} and if $A \rightarrow X$ is a small fibration then any of its pullback is also a small fibration.  \qedhere
\end{enumerate}
\end{proof} 



The simplicial sets $\U$ and $\UU$ do not appear to be cofibrant. However, we can consider a cofibrant replacement of $\U$, given by a cofibrant object $\U_c$ and a trivial fibration
\begin{equation}
\label{equ:ucu}
...
simplicial set $\mathsf{T}_c(a)$ given by the pullback
\[
\xymatrix{
\mathsf{T}(a) \ar[r] \ar[d]_p & \UU_c \ar[d]^\pi \\
X \ar[r]_a & \U_c }
\]
is cofibrant.
...
\begin{proof} \hfill
\begin{enumerate}[(i)] 
\item The claim follows from part~(ii) of~\cref{thm:universe-u}. 
\item Easy.
...
We now prove that the small fibration $\pi_c \co \UU_c \to \U_c$ is univalent. 
Let $\U_c^{\rightarrow}$ be the simplicial set whose $n$-cells are given by triples $(F_0, F_1, \phi)$ where
 $F_0$ and $F_1$ are $n$-cells of $\U$, \ie functors
 \[
F_0, F_1 \co {\Delta_{/[n]}}^{\op} \rightarrow \Set
\]
and $\phi \co F_0 \Rightarrow F_1$ is a natural transformation. We define the subobject $\mathsf{Weq}(\U_c) \subseteq \U_c^{\rightarrow}$ as consisting of the 
$n$-cells $(F_1, F_2, \phi) \co \Delta[n] \rightarrow \U_c^{\rightarrow}$ such that in the corresponding diagram:
\[
\xymatrix{ 
A_1 \ar[rr]^{w} \ar@{->>}[dr]_{p_1} & & A_2 \ar@{->>}[dl]^{p_2} \\
& \Delta[n] & }
\]
the map $w$ is weak equivalence. The simplicial set $\mathsf{Weq}(\U_c)$ is well defined because for any map $f \co X \rightarrow Y$ is the pullback functor $f^* \co \SSet_{/Y} \rightarrow \SSet_{/X}$ is a right Quillen functor \cite{??} and therefore preserves weak equivalences. Hence, 
$\mathsf{Weq}(\U_c)$ as defined here is indeed a subobject of $\U_c^{\rightarrow}$. \caution{To be checked again!}

\begin{proposition}
\label{prop:Weq_classify_Weq}
For any cofibrant object $X$, a map $a \co X \rightarrow \U_c^{\rightarrow}$ factors via 
 $\mathsf{Weq}(\U_c)$ if and only the map in $\SSet_{/X}$ classified by $a$,
\[
\xymatrix{
A_1 \ar[rr]^w  \ar[dr]_{p_1} & & A_2 \ar[dl]^{p_2} \\
 & X \, , & }
...
\begin{proof} By definition of $\mathsf{Weq}(\U_c)$, $a$ factors via $\mathsf{Weq}(\U_c)$ if and only if the pullback of $w \co A_1 \to A_2$ along any simplex $x \co \Delta[n] \rightarrow X$  is a weak equivalence. \caution{As observed above, this is indeed the case if $w$ is a weak equivalence.} Conversely,
...
\item The simplicial sets $\U_c$ and $\mathsf{Weq}(\U_c)$ are fibrant.
\end{enumerate}
\end{proposition}
