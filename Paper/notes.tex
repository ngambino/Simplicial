...
\begin{enumerate}[$(i)$]
\item A map $f \co Y \to X$ in $\SSet$ is a cofibration if and only if it is a levelwise complemented monomorphism such that the degeneracy of simplices is decidable on the complement of the image of $f$. 
...
Let $Y$ and $X$ be cofibrant. Then for every pullback of the form
\[
\xymatrix{
Y \times_X Z \ar[r] \ar[d] & Z \ar[d] \\
Y \ar[r] & X }
\]
the simplicial set $Y \times_{X} Z$ is cofibrant.
\end{lemma}
...
\begin{proposition} \label{thm:cof-pbk} Let $X ,Y$ and $Z$ be cofibrant simplicial sets, $f \co Y \rightarrow X$ a cofibration and
\[
\xymatrix{
Y \times_X Z \ar[r] \ar[d] &  Z \ar[d] \\
Y \ar[r]_f & X}
\]
be a pullback diagram. Then $Y \times_X Z \rightarrow Z$ is a cofibration.
\end{proposition}
...
Let $p \co A \rightarrow X$ be a fibration with cofibrant domain and $q \co A.B \rightarrow A$ a fibration. Then $\Pi_p(q) \co \Pi_A(B) \rightarrow X$ is a fibration.
\end{corollary}
