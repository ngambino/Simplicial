...
chosen fillers for lifting problems and is required to satisfy some naturality conditions. While 
categories of cubical sets considered in this work allow us to define constructively models of HoTT 
and often admit a Quillen
...
fibrations are the Kan fibrations.  Crucially, the cofibrations in this constructive model structure are
...
The present paper extends this work to obtain constructive counterparts of all the other main results of simplicial
...
These results allow us to define a constructive interpretation of HoTT in the category of simplicial sets that supports identity types, $\Sigma$-types, $\Pi$-types and a type-theoretic universe satisfying the univalence axiom. In this new interpretation, type-theoretic
contexts are mapped into bifibrant objects, \ie cofibrant Kan complexes.   Dependent types are interpreted as fibrations $p \co A \to X$ where $X$ is a cofibrant Kan complex and $A$ is cofibrant. This choice is informed by the fact that, for a simplicial set~$X$, the slice category~$\SSet_{/X}$ admits a model structure in which fibrations and cofibrations are the maps that are fibrations and cofibrations in~$\SSet$, respectively. Then, the bifibrant objects in~$\SSet_{/X}$ are the fibrations $p \co A \to X$ with cofibrant domain. Thanks to~\cref{thm:MainPathObject}, identity types can be interpreted as path spaces. The classical and the constructive interpretations of HoTT in simplicial sets are compared in~\cref{tab:interpretations}. 
...
For the convenience of the reader, we recall the main aspects of this model structure.
...
Given a map $p \co Y \to X$ in $\SSet$, we write 
\[
f^* \co \SSet_{/X} \to \SSet_{/Y}
\] 
for the associated pullback functor. Since $\SSet$ is locally cartesian closed, the pullback functor 
$f^*$ has both a  left and a right adjoint, written $\Sigma_{f} \co \SSet_{/Y} \to \SSet_{/X}$ and
$\Pi_f \co \SSet_{/Y} \to \SSet_{/X}$, respectively.   We call these functors the \emph{dependent sum} and \emph{dependent
...
\[
 ...
 \]
respectively. Since  $\Sigma_f$ is defined by composition, $\Sigma_Y(Z) \defeq Z$ and $\Sigma_f(g) = g f $. We denote the action of $f^*$ on Kan fibration $p \co A \to X$ as
\[
\xymatrix{
...
Y \ar@{>->}[r]_{f} \ & X}
\]
Since $A$ is cofibrant, the claim is that the monomorphism $A[f] \rightarrowtail A$ is a levelwise complemented. For $a \in A_n$, we have that $a \in A[f]_n$ if and only if $p(a) \in Y_n$. Since $f \co Y \rightarrowtail X$ is a levelwise complemented monomorphism, this is decidable.
\end{proof} 
...
The aim of this section is to show that, for a  fibration $p \co A \to X$ with cofibrant domain, we can
use path spaces to obtain a factorisation of the diagonal $\delta_p \co A \to A \times_X A$ as a trivial cofibration followed by a fibration. This allows us to show that identity types can be interpreted
as path spaces, \cf~\cite{awodey-warren:homotopy-idtype}.
...
There are evident boundary maps $\partial \co \Path(A) \to A \times A$, giving
the endpoints of a path. We write $\partial_0$, $\partial_1$ for the compositions of $\partial$
...
By a similar argument, for $k=0 \, , 1$, the map $\langle \Lambda^k[n] \hookrightarrow \Delta[n] \, , A \rightarrow X\rangle$ is a trivial fibration. Indeed, this is the map
\[ 
...
The main goal of this section is to show that a Kan fibration $p \co A \rightarrow X$ with cofibrant domain, the dependent product functor
\[
\Pi_p \co \SSet_{/A} \to \SSet_{/X}
\]
preserves Kan fibrations. As we will see, this result allows us to interpret $\Pi$-types via a cofibrant replacement. By adjointness, our goal is equivalent to showing that the pullback functor
\[
f^* \co \SSet_{/X} \to \SSet_{/A}
\]
preserves trivial cofibrations, which is restricted version of the Frobenius property~\cite{BergB:topsmi}.
Here, the restriction amounts to the fact that we are considering pullbacks along Kan fibrations whose 
domain is cofibrant, rather than general Kan fibrations.. As we will see, some of these assumptions are essential in our
setting.  

Our proof of the restricted Frobenius property follows essentially~\cite{gambino2017frobenius}. 
In order to combine our development with that argument, however, we need to show that the notion of a Kan fibration considered here coincides with the one considered in~\cite{gambino2017frobenius}. For this, 
...
\item The map $p$ has the right lifting property with respect to the pushout products $\kcyl  \hattimes h^n_i$, 
where $k \in \braces{ 0 \, , 1 }$ and $h^n_i \co \Lambda^i[n] \to \Delta[n]$  is a horn inclusion.
\end{enumerate}
\end{proposition} 
...
\begin{proof} Since the pullback functor $p^*$ has a right adjoint,  it is enough to check that pullbacks of generating trivial cofibrations are trivial cofibrations, respectively. For trivial cofibrations, note that the trivial cofibrations in $\SSet_{/X}$ are generated by the horn inclusions $h^k_n \co \Lambda^{k}[n] \rightarrow \Delta[n]$ for all possible choices of $x \co \Delta[n] \rightarrow X$. By \cref{lemma:genTcof_strongHequiv} they are all strong $k$-oriented homotopy equivalences. Moreover, their pullback to $A$ is also their pullback along the map $A \times_X \Delta[n] \rightarrow \Delta[n]$ which is again a fibration with cofibrant domain by \cref{lem:cofibrant_fiber_product}. Hence  \cref{lemma:pb_of_StrongHomotopyEq} implies that the pullback 
...
Let $p \co A \rightarrow X$ be a fibration with cofibrant domain. The functor $\Pi_p \co \SSet_{/A} \to \SSet_{/X}$ preserves  fibrations.
\end{theorem}
...
cofibrant objects. 
\begin{enumerate}[$(i)$]
\item The dependent product functor $\Pi_f \co \SSet_{/Y} \rightarrow \SSet_{/X}$ preserves trivial fibrations.
\item The counit of the adjunction $f^* \dashv \Pi_f$ is a natural isomorphism.
\item If $p \co A \to Y$  is cofibrant in $\SSet_{/Y}$, then $\Pi_f(p) \co \Pi_Y(A) \to X$  is cofibrant 
in~$\SSet_{/X}$.
...
\caution{[To be checked or simplified]} For part (iii),   let $v$ be a $n$-cell in $\Pi_Y(A)$.
