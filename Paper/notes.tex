...
\begin{proof}
This follows from corollary 3.1.11 in \cite{henry2019qms}. \notesh{I don't know if more details is needed. I've written them anyway, if you think they are useless remove them.}
The first point say that if $K$ is a finite (decidable) simplicial set and $A$ is cofibrant then $A^K$ is cofibrant, which implies that $\Path(A)= A^{\Delta[1]}$ is cofibrant, the second points say that when $f:K \rightarrow K'$ is a ``degeneracy quotient'' between finite simplicial sets, which is in particular the case of the map $\Delta[1] \rightarrow \Delta[0]$ then the induced map $A^{K'} \rightarrow A^{K}$ is a cofibration, this gives that $r \co A \rightarrow \Path(A)$ is a cofibration.
...
For part (iii), we need to use the machinery of ``degenracy quotient'' introduced in section 3.1 \cite{henry2019qms}. Let $x: \Delta[n] \rightarrow \Pi_Y A$ be a  $k$-cell, we will show that for a given degeneracy $\sigma:[n] \rightarrow [k]$ it is decidable if $x$ is ``$\sigma$-degenerated'', i.e. if $x$ factors through $\Delta[k]$. As degeneracy is decidable in $X$, one can freely assume that the image of $x$ in $X$ is $\sigma$-degenerated hence one has a solid diagram:
...
The objects $f^* \Delta[n]$ and $f^* \Delta[k]$ are decidable simplicial subset of $\Delta[n]$ and $\Delta[k]$ because $f$ is itself a levelwise complemented monomorphism, hence they are both finite decidable simplicial sets. The map $f^* \sigma$ is what is called in \cite[section 3.1]{henry2019qms} a ``degeneracy quotient'' (because of proposition 3.1.11 loc.cit.) and lemma 3.1.8 loc.cit. shows that the existence of such a lift against a degeneracy quotient between finite simplicial sets, with $A$ cofibrant is decidable (because degeneracy quotients are epimorphisms, the existence of lift making the upper triangle commutes is equivalent to the existence of lift making the square commutes).
...
Conversely,
we let $w \co A_1 \rightarrow A_2$ be a map between bifibrant objects of $\SSet_{/X}$, assume that the pullback of $w$ along every $x \co \Delta[n] \rightarrow X$ is a weak equivalence and show that $w$ is also equivalence. One factors $w = p j$ as a trivial cofibration $j$ followed by a fibration $p$ and we will show that the fibration is a trivial fibration. For any $x \co \Delta[n] \rightarrow X$, the pullback of $x^* w$ is an equivalence by assumption and the pullback of $x^* j$ is an equivalence by the first part of the proof, hence by two-out-of-three, the pullback of $x^* p$ is also an equivalence, and hence a trivial fibration.

To conclude, let $p : Y_1 \rightarrow Y_2$ be a fibration in $\SSet_{/X}$ such that for all cell $x: \Delta[n] \rightarrow X$ the morphism $x^* p \co x^* Y_1 \rightarrow x^* Y_2$ is a trivial fibration, we show that $p$ is a trivial fibration. Indeed, given any lifting problem of $\partial \Delta[n] \rightarrow \Delta[n]$ against $p$, one can consider the composite of $x: \Delta[n] \rightarrow Y_2 \rightarrow X$, take the pullback $x^* p$ of $p$ along this map, and using that $x^* p$ is a trivial fibration one obtain a lift.

\notesh{I'll maybe add a diagram here later.}
