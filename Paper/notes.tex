...
This paper investigates further Voevodsky's model of Martin-L\"of type theory in the category of simplicial sets~\cite{voevodsky-simplicial-model}.
This model is of fundamental importance since it guided the the Univalent Foundations of Mathematics programme~\cite{voevodsky:library} and
the more general area of Homotopy Type Theory~\cite{hottbook}. In particular, the simplicial model suggested a new approach to the formalisation of mathematics in type
theory, informed by the possibility of defining the notion of the homotopy level of type, and the formulation of the Univalence Axiom, which makes precise the idea that isomorphic structures should be considered as equal. 

The original definition of the simplicial model  was carried out working within classical Zermelo-Fraenkel set theory extended with the axiom of choice (ZFC) and two inaccessible cardinals. Here,~ZFC was used as the background theory to prove the results of simplicial homotopy theory
necessary to set up the interpretation, one inaccessible cardinal  to interpret a type-theoretic universe and the other
 to address coherence issues of the interepretation. The question of whether the simplicial model could be defined working constructively, \ie without the use of the law of excluded middle or the axiom of choice,  was one of the
 central questions investigated during the thematic programme on Univalent Foundations at the Institute for Advanced Study in 2012/13
and has remained one of the main open problems in the area since then.

The aim of this paper is to provide a partial solution to this long-standing open problem.  In order to
...
to define the simplicial model of Martin-L\"of type theory extended with the Univalence Axiom are 
...
it is not possible to prove constructively that for all simplicial sets $A$ and 
Kan complexes $B$, the exponential $B^A$ is again a Kan complex.
Because of this, Coquand and several collaborators  developed homotopy-theoretic models of type theory in categories of cubical sets~\cite{coquand-cubical-sets}, starting a very profitable line of enquiry, cf.~\cite{awodey-cubical,PittsAM:aximct}. On top of switching 
...
chosen fillers for lifting problems. While they
succeeded in interpreting constructively type theory with univalence in cubical sets, none of the
categories of cubical sets considered are known to be Quillen equivalent to simplicial sets or topological spaces.

As shown in~\cite{gambino2017frobenius}, it is possible to develop constructively a theory of uniform fibrations
also in simplicial sets and prove that the dependent product functor along a uniform fibration preserves
uniform fibrations, showing in particular that that for an algebraic Kan complex $B$, the simplicial set $B^A$ is an algebraic Kan complex for every simplicial set $A$. However, the notion of a uniform fibration
in simplicial sets considered in~\cite{gambino2017frobenius} is not as well-behaved as the one in cubical sets,
in that it does not admit a universal uniform fibration~\cite{SattlerC:unpublished}, which is ultimately
due to the failure of representables to be closed under under products. This situation is summarised in~\cref{tab:whack}.
...
and for which the degeneracy of $n$-simplices is decidable in $B_n \setminus A_n$, for every $[n] \in \Delta$. In particular, the cofibrant objects are in this model structure are not all simplicial sets, but only those with decidable degeneracies. Thus, this model structure coincides with
the standard one as soon as the law of excluded middle is assumed. Another proof of the constructive existence of this
model structure is obtained in~\cite{GambinoN:anocp}, without relying on the results in~\cite{henry2018wms}. 


This paper builds on this work to obtain constructive counterparts of all the main results of simplicial
homotopy theory in~\cite{voevodsky-simplicial-model} necessary to define the simplicial model of Martin-L\"of type theory extended with the
Univalence Axiom. In particular, our main results are the following:
\begin{itemize}
...
\item \cref{cor:Pi_types_are_fibrant}, asserting that for a fibration $p \co A \to X$ with cofibrant
domain the dependent product functor $\Pi_p \co \SSet_{/A} \to \SSet_{/X}$ preserves fibrations;
...
\item \cref{thm:fibrancy-of-u-and-uc}, asserting that $\UU_c$ is both fibrant and cofibrant;
\item \cref{thm:univalence-of-u-and-uc}, asserting that the universal fibration $\pi_c  \co
\UU_c \to \U_c$ is univalent.
\end{itemize}
Overall, these contributions provide a further column to~\cref{tab:whack}, which is displayed
...
\end{tabular}
\medskip
\caption{The main contributions of this paper.} 
\label{tab:contribs}
\end{table}

The proofs of these results are inspired by similar results existing in the literature, especially~\cite{voevodsky-simplicial-model} and~\cite{gambino2017frobenius}, but require a systematic
and careful analysis of the cofibrancy assumptions needed to carry over these proofs. This is especially important in the case of the results
for universes, for which we are not interested in a small fibration that classifies small fibrations as in~\cite{voevodsky-simplicial-mode}, but
...
Our results allow us to define a new interpretation of Martin-L\"of type theory into the category of simplicial sets. The outline
of this interpretation and its differences with the standard one are presented in~\cref{tab:interpretations}. In this interpretation, type-theoretic
contexts are mapped into bifibrant simplicial sets. In particular, the empty context is interpreted as the terminal simplicial set $1$.  Dependent types are interpreted as fibrations $p \co A \to X$ where $X$ is  bifibrant and $A$ is cofibrant. This choice is informed by the fact that, for a bifibrant simplicial set $X$, the slice category~$\SSet_{/X}$ admits a model structure in which fibrations and cofibrations are the maps that become fibrations and cofibrations, respectively, in~$\SSet$. Then, the bifibrant objects in $\SSet_{/X}$ are exactly the fibrations $p \co A \to X$ with cofibrant domain. Then, a type in the empty context is interpreted as a bifibrant simplicial set. 
...
Following this idea, identity types are interpreted as path spaces, as desired in Homotopy Type Theory. Indeed, for a fibration $p \co A \to X$ with cofibrant domain, \cref{thm:MainPathObject} shows that the factorisation of the diagonal $\delta_p \co A \to A \times_X A$ via the mapping path space
\[
\xymatrix{
A \ar[rr]^{\mathrm{refl}_p}  \ar[dr]_{\delta_p} & & \Path(p) \ar[dl]^{\partial} \\
  & A \times_X A &  \, ,}
\]
is a factorisation of $\delta_p$ as a trivial cofibration followed by a fibration.  A subtle issue arises when considering the interpretation of $\Pi$-types. Indeed, for a fibration $p \co A \to X$ with cofibrant domain and , the result of applying the dependent product functor $\Pi_p$ a fibration $q \co B \to A$ with cofibrant domain is a map $\Pi_p(q) \co \Pi_A(B) \to X$, which is a fibration by~\cref{cor:Pi_types_are_fibrant} but its domain
...
$\tilde{\Pi}_p(q) \co \tilde{\Pi}_A(B) \to X$, which is now a fibration with cofibrant domain. Interestingly, this definition validates 
a judgemental $\beta$-rule and a propositional $\eta$-rule, a combination that arises naturally when  Martin-L\"of type theory is presented
...
library~\cite{voevodsky:library}, including his proof that the Univalence Axiom implies the principle of Function Extensionality, was originally developed assuming only a propositional $\eta$-rule.

For the interpretation of a univalent type universe, we shall proceed in two steps, using again a cofibrant replacement in the second step. 
...
We refrain from claiming that our work provides a complete solution to a constructive version of the simplicial model of type theory
in that the interpretation outlined does not validate strictly the rules of type theory for substitution. These rules come in two groups:
...
there are well-known techniques to address this issues~\cite{voevodsky-simplicial-model}, they do not seem to apply in our context.
...
we will use throughout the paper. \cref{sec:pats} shows how path spaces fit into the (trivial cofibration,
fibration) factorisation. \cref{sec:Pi-types} proves the restricted Frobenius property. \cref{sec:equep}
establishes the weak equivalence extension property. \cref{sec:unifbb} introduces the universal
fibration $\pi_c \co \UU_c \to \U_c$. \cref{sec:fibrancy-and-univalence} proves that $\U_c$ is bifibrant and that $\pi_c$ is univalent. 
