...
\begin{enumerate}[$(i)$]
\item Let $A \rightarrow B$ be a degeneracy quotient and $X$ any simplicial set. Then $A \times X \rightarrow B \times X$ is a degeneracy quotient.
\item Let $X$ be cofibrant and $K$ finite and decidable. Then $X^K$ is cofibrant.
\item Let $A \rightarrow B$ be a degeneracy quotient of finite decidable simplicial sets and $X$  cofibrant. Then the induced map $X^B \rightarrow X^A$ is a cofibration between cofibrant objects.
\end{enumerate}
\end{corollary}
...
the rule for $\Pi$-types in~\cref{tab:Pi-types}. Here, note that we are considering the propositional
version of the $\eta$-rule.
