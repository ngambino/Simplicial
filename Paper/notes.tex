...
to as the \emph{dependent sum} along $f$. Since $\SSet$ is locally cartesian closed, the pullback functor 
$f^*$ has also a right adjoint, which we write 
...
Below, we will show that, for a Kan fibration $p \co A \rightarrow X$ with cofibrant domain, the dependent product functor $\Pi_p$ preserves Kan fibrations, without assuming $X$ to be a cofibrant Kan complex. By adjointness, our goal is equivalent to showing that its left adjoint, \ie 
the pullback functor $f^*$, preserves trivial cofibrations. This amounts to proving a restricted version of the Frobenius property~\cite{BergB:topsmi}, obtained by considering pullbacks along Kan fibrations with
...
\section{A versal fibration for small fibrations between bifibrant objects}
\label{sec:unifbb}
...
$\pi_c$ to be a fibration that is versal for the class of small fibrations between cofibrant simplicial
...
X \ar[r]_-a &  \U_c \, .}
\]
Here, we speak of a versal rather than universal fibration since we make no requirement for
...
is versal for small fibrations between cofibrant simplicial sets and leave the proofs
