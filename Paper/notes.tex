...
Let $\catE$  be a category. For $X \in \catE$ we write $\catE_{/X}$ for the slice category over
$X$. Given a map $p \co A \to X$, we write $\Sigma_p \co \catE_{/A} \to \catE_{/X}$ for the composition
functor and denote its action on an object $q \co B \to A$ as $\Sigma_p(q) \co \Sigma_A(B) \to X$. When
$\catE$ has pullbacks, $\Sigma_p$ has a right adjoint, which we denote $p^* \co \catE_{/X} \to \catE_{/A}$,
whose action will be written as follows
\[
\xymatrix{
A[f] \ar[r] \ar[d] & A \ar[d]^p \\
Y \ar[r]_f & X }
\]
When $\catE$ is locally cartesian closed, the pullback  $p^*$ has a further right adjoint, which we denote 
$\Pi_p \co \catE_{/A} \to \catE_{/X}$. Its action on $q \co B \to A$ will be written $\Pi_p(q) \co 
\Pi_A(B) \to X$. For $p \co A \to X$, the result of applying $\Pi_p$ to a fibration with cofibrant domain $q \co B \to A$ will be written as~$\Pi(q) \co \Pi_A(B) \to X$. 
...
We say that a simplicial set is \myemph{bifibrant} if it is both fibrant and cofibrant.  We write $\Fib_{/X}$ for the full subcategory of the slice category $\SSet_{/X}$ spanned by the fibrations with codomain $X$. Of key interest for us will be the full subcategory of  $\Fib_{/X}$ spanned by fibrations with cofibrant domain, which we denote by  $\BFFib_{/X}$.
...
the composite $\Sigma_p(q) \co \Sigma_A(B) \to X$ is again a fibration with cofibrant domain.
\end{remark} 



\section{Identity types as path objects}
