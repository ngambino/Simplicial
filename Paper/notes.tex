...
We also make use of another weak factorisation system $(\mathsf{L}, \mathsf{R})$ on $\SSet$ introduced in~\cite[???]{henry2019qms}. This is useful to establish decidability conditions; in particular, we will use it to prove~\cref{proposition:PathObjectCofibrant} and~\cref{Lemma:ForTheExtProperty} here. By definition, $(\mathsf{L}, \mathsf{R})$ is the
...
\begin{remark} We conclude this section by defining the comprehension category 
\begin{equation}
\label{equ:compcat}
\begin{gathered}
\xymatrix{
\mathbf{Fib}_{ \mathsf{cof}} \ar[dr]_{p} \ar[rr]^{\chi} & & \SSet^{\to}_{\mathsf{cof}} \ar[dl]^{\mathrm{cod}} \\ 
 ...
 \end{gathered}
 \end{equation}
that will be  shown to support various type-theoretic constructors in the rest of the paper. We refer to~\cite{JacobsB:catltt,LumsdaineP:locuoc} for the definition of a comprehension category 
and related notions. The category $\SSet_{\mathsf{cof}}$ that is the base  of the comprehension category in~\eqref{equ:compcat} is defined to be the full subcategory of $\SSet$ spanned by cofibrant simplicial sets. The category $\SSet^\to_{\mathsf{cof}}$ is the associated arrow category, whose objects are
maps $p \co A \to X$ between cofibrant objects. The codomain
functor 
\[
\mathrm{cod} \co \SSet^{\to}_{\mathsf{cof}} \to \SSet_{\mathsf{cof}}
\] 
is a Grothendieck fibration by part~(ii) of~\cref{lem:cofibrant_fiber_product}. Finally, $\mathbf{Fib}_{ \mathsf{cof}}$ is the full subcategory of $\SSet^{\to}_{\mathsf{cof}}$ spanned by fibrations. Thus,
its objects are fibrations $p \co A \to X$ between cofibrant objects. The comprehension $\chi$ is
defined to be the inclusion, so that $p = \mathrm{cod} \circ \chi$. Since $\chi$ preserves pullbacks,
we have a comprehension category. 

It immediate to observe that this comprehension category supports $\Sigma$-types, since for
a fibration $p \co A \to X$ between cofibrant objects, dependent sum along $p$ 
functor~$\Sigma_p \co \SSet_{/A} \to \SSet_{/X}$, which is defined by
...
In this section we begin to show how the the comprehension category of~\eqref{equ:compcat} supports 
various type-theoretic constructs by considering identity types. Following the fundamental insight in~\cite{awodey-warren:homotopy-idtype}, in order to equip the comprehension category of~\eqref{equ:compcat} with identity types it suffices to show that
...
the comprehension category of~\eqref{equ:compcat} supports $\Pi$-types. 
