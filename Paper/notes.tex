...
We will make use also of another weak factorisation system $(\mathsf{L}, \mathsf{R})$ on $\SSet$, which is useful to prove
decidability conditions and which we use to prove~\cref{proposition:PathObjectCofibrant} and 
~\cref{Lemma:ForTheExtProperty} here. By definition, $(\mathsf{L}, \mathsf{R})$ is the
...
instead the pushouts of coproducts of degeneracy maps, \ie the maps of the form $X \rightarrow X[(x_i,\sigma_i])$ where $x_i \in X_{n-1}$ are a family of cells,  $\sigma \co [n_i] \twoheadrightarrow [m_i]$ a family of degeneracies and $X[(x_i,\sigma_i)]$ is obtained from from $X$ by freely making $x_i$ in the image of $\sigma_i $ for each $i$. For the convenience of the reader, we recall
 the results in \cite{henry2019qms} about this factorization system that will be used here.
...
The proof of the result above is sufficiently simple  that it can be outlined in a few words. Since $B = A[(a_i,\sigma_i)]$ for a finite collection of cells $a_i$,  $f$ factors through $p$ if and only if $f(a_i) =\sigma_i^*  x_i$ for each $i$, but when $X$ is cofibrant one can decide for each $i$ if this is the case or not. As there is only a finite number of such $i$, one can decide if it is the case for all $i$ or not.
...
\begin{corollary} \label{prop:X^kCofibrant}  \hfill 
\begin{enumerate}[$(i)$]
\item If $A \rightarrow B$ is a degeneracy quotient and $X$ any simplicial set then $A \times X \rightarrow B \times X$ is a degeneracy quotient.
...
\begin{proof} Part~(i) is a special case of \cref{prop:Degen_quotient_are_pullback_stable}. For part~(ii), recall that an $n$-cell $\Delta[n] \rightarrow X^K$ is a morphism $\Delta[n] \times K \rightarrow X$. Thus, for a degeneracy map $\Delta[n] \rightarrow \Delta[m]$, the map $\Delta[n] \times K \rightarrow \Delta[m] \times K$ is a degeneracy quotient by part (i), as it is a map between finite decidable simplicial sets. Thus, \cref{lem:decidability_lift_degen_quo} implies that the question of whether a map $\Delta[n] \times K \rightarrow X$ factor through $\Delta[n] \times K \rightarrow \Delta[m] \times K$ is also decidable. But this is exactly the question of degeneracy of cells in $X^K$.

Similarly, for part (iii), since an $n$-cell of $X^A$ is a morphism $\Delta[n] \times A \rightarrow X$, it belongs to $X^B$ if and only it can be factored as $\Delta[n] \times A \rightarrow \Delta[n] \times B \rightarrow X$. This is decidable by \cref{lem:decidability_lift_degen_quo} because $\Delta[n] \times A \rightarrow \Delta[n] \times B$ is a degeneracy quotient between finite decidable simplicial sets by part (i).
\end{proof} 
