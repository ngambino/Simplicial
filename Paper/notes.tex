...
\item a pseudo-stable type of natural number $\mathbb{N}$,
\item pseudo-stable $+$-type,
 
\item a pseudo Tarski universe $\pi_c: \UU_c \rightarrow \U_c$, containing $\mathbb{N}$ and closed under the $\Sigma$-types, $\Id$-types, $\Pi$-types and $+$-types constructions.
\end{itemize}
...
The type of natural number is simply given by the discrete simplicial sets of natural number object, which is cofibrant (every cell of dimension $>0$ is degenerated) and fibrant. The $+$ type $A + B$ is also given by the disjoint coproduct of simplicial sets, which is automatically fibrant and cofibrant as well.
...
Note that a careful analysis of how the cofibrant replacement of a simplicial set is constructed in appendix \ref{sec:appendix} allows to show (see part~$(i)$ of \cref{cor:cofibrant_smallness}) that given any simplicial sets $X$ the map $\mathbb{L} X \rightarrow X$ is always a small fibration independently of whether $X$ is small or not. Hence the construction of $\Pi$-types really preserves small fibrations, even between non-small objects.
\end{proof}
...
\begin{itemize}
\item If one do not make this assumption that subsets of small sets are small, and keep all the definition the same, one still has a contextual category with all the structure mentioned above, but one looses in general that the universe is stable under the construction of $\Pi$-types. However, one still has (in type theoretic notation) that if: $ \Gamma,a : A \vdash B(a) : \U_c$ then $\Gamma \vdash \Pi_A B : \U_c$ as long as $\Gamma$ is a context only involving small types (while in MLTT this would holds for any context).

\item One can work in CZF without assuming any innacessible sets, and move to an interpretation where contexts are ``simplicial classes'' (where classes are formulas), general types are given by Kan fibrations between simplicial classes and a small fibrations is one whose fibers are sets. In this case part~$(ii)$ of \cref{cor:cofibrant_smallness} that produces the smallness property that ensure that the cofibrant replacement preserves small type even in non-small context. In this case one does have that the universe is stable under $\Pi$-type (as well as all the other constructor) but the most general kind of $\Pi$-types cannot be constructed: one can form $\Pi_A B$ only when $A$ is a small type (i.e. an element of the univers).
\end{itemize}
