...
For  $X \in \SSet$ we write $\SSet_{/X}$ for the slice category over~$X$. For a map $f \co Y \to X$, we write 
$f^* \co \SSet_{/X} \to \SSet_{/Y}$ for the associated pullback functor and denote its action on $p \co A \to X$ as 
\[
\xymatrix{
f^*(A) \ar[r] \ar[d]_{f^*(p)} & A \ar[d]^p \\
Y \ar[r]_f & X }
\]
This functor has both a left adjoint $\Sigma_f \co \SSet_{/Y} \to \SSet_{/X}$, defined by composition with $f$,
and a right adjoint $\Pi_f \co \catE_{/Y} \to \catE_{/X}$. Their action on a
map $q \co B \to Y$ will be written
\[
 \Sigma_f(q) \co \Sigma_Y(B) \to X \, , \quad \Pi_f(q) \co \Pi_Y(B) \to X \, ,
 \]
respectively. Since the left adjoint is defined by composition, $\Sigma_Y(B) \defeq B$ and $\Sigma_f(q) = qf$.
...
\end{equation}
For $F \co {\Delta_{/[n]}}^{\op} \to \Set$, we write $\pi_1 \co \int F \to \Delta[n]$
