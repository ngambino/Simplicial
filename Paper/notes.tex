...
The simplicial set $\Delta[1]$ is an interval object in $\SSet$, with endpoint inclusions~$\kcyl \co \braces{ k} \to \Delta[1]$ defined by~$\kcyl \defeq h^1_k$.
...
We shall work  with the model structure on $\SSet$ introduced  in \cite[Section~5]{henry2018wms}, which we now recall. 
...
It was shown in~\cite{Henry2019} that two weak factorisation systems
$(\Cof, \TrivFib)$ and $(\TrivCof, \Fib)$ are part of a model structure on the category $\SSet$.
Furthermore, it was also shown in~\cite{Henry2019} that the model structure is both 
...
\caution{subobject} $\mathsf{Weq}(\U)$ if and only the map in $\SSet_{/X}$ classified by the map $a$ is a weak equivalence.
