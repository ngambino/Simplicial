...
shall concentrate on the weak factorisation system of fibrations and trivial cofibrations. For this, we shall
follow essentially~\cite{gambino2017frobenius}. In order to do so, however, we need the following 
preliminary proposition, in which we use the pushout product $\kcyl \hattimes f$ of an endpoint inclusion $\kcyl \co \braces{ k } \to \Delta[1]$ and a map~$f \co Y \to X$, which is defined as the unique dotted arrow  in the diagram:
\[
\xymatrix{
...
For any cofibrant object $X$, a morphism $a \co X \rightarrow \U_c^{\rightarrow}$ factors via the 
\caution{subobject} $\mathsf{Weq}(\U_c)$ if and only the map in $\SSet_{/X}$ classified by the map $a$ is a weak equivalence.

\end{proposition}

\begin{proof} Given $a \co X \rightarrow \U_c^{\rightarrow}$, let  
\[
\xymatrix{
A_1 \ar[rr]^w  \ar[dr]_{p_1} & & A_2 \ar[dl]^{p_2} \\
 & X & }
 \]
be the diagram classified by $a$.  By definition of $\mathsf{Weq}(\U_c)$, $a$ factors via $\mathsf{Weq}(\U_c)$ if and only if the pullback of $w \co A_1 \to A_2$ along any simplex $x \co \Delta[n] \rightarrow X$  is a weak equivalence. \caution{As observed above, this is indeed the case if $w$ is a weak equivalence.} Conversely,
we let $w \co A_1 \rightarrow A_2$ be a map between bifibrant objects of $\SSet_{/X}$, assume that the pullback of $w$ along every $x \co \Delta[n] \rightarrow X$ is a weak equivalence and show that $w$ is also equivalence.  We do so using  of \cite[Theorem 2.5.6]{henry2018wms} or, more precisely its reformulation in \cite[\S 2.5.7]{henry2018wms}, and show that $w$ has the weak right lifting property against all $i_n \co \partial \Delta[n] \rightarrow \Delta[n]$. So let us consider the diagram
\begin{equation}
...
\end{gathered}
\end{equation}
By pulling back everything to $\Delta[n]$ we obtain
\begin{equation}
