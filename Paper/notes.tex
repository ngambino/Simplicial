...
NOTE: I remember we proved $(iii)$ explicitely as this was something I was worried about. but the following proof is a lot harder than in my memories. Do you have any notes about this claims ? If not just erase this NOTE.
...
\item Let $v$ be a $n$-cell in $\Pi_i(X)$. If the image of $v$ is in $A$ then $v$ is a cell of $X \subset \Pi_i(X)$ and hence it is decidable whether $v$ is degenerate or not.
So one can assume that $v$ is not in the image of $A$, hence it is decidable if the image of $v$ in $B$ is degenerate or not. By the Eilenberg-Zilber lemma one can also decide for each given degeneracy if the image of $v$ is degenerate for this precise degeneracy or not. 

Let $\sigma:[n] \twoheadrightarrow [k]$ be any degeneracy, we will show that it is decidable whether $v$ is ``$\sigma$-degenerate, i.e. if $v =\sigma^* v'$ for some $v'$. If this is the case, then such a $v'$ is neccesarilly $j^* v$ for $j$ any section of $\sigma$, and hence this implies that the image of $v$ in $B$ is $\sigma$-degenerate as well. Hence one can freely assume that the image of $v$ in $B$ is a $\sigma^* b$ for some $b \in B$ (and not in $A$).
...
Given its image in $B$, the cell $v: \Delta[n] \rightarrow \Pi_i X$ is uniquely determined by the data of a morphism $\lambda: V\rightarrow X$, it is $\sigma$-degenerate if and only if it factors into $V_{\sigma}$. Note that such a factorization is unique. For $J \subset [n]$, the $J$-face of a cell is said to be $\sigma$-degenerate if and only if it is degenerate for the (potentially trivial) degeneracy: $\sigma_{|J} : J \rightarrow \sigma(J)$. But one can see that $\lambda$ factor into $V_{\sigma}$ if and only for all $i:[f] \hookrightarrow [n]$ that belong to $V$ (and $V$ is decidable so there is only a finite cardinal of them), $i^* \lambda$ is degenerate at $\sigma$. Indeed $V$ is the gluing of all the $\sigma \circ i$ for such faces, for each individual face $i$ one has a factorization into its image in $V_{\sigma}$ if and only if $i^* \lambda$ is $\sigma$-degenerate, and as such factorization are unique they patch together on $V_{\sigma}$ is they all exists.

\item One can simply take $p'= \Pi_i p$: it is a trivial fibration by point $(i)$ and the square is a pullback by point $(ii)$. The final remark about the cofibracancy of $Y'$ follow from $(iii)$.
