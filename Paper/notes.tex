...
The aim of this paper is to advance our understanding of the connections between Martin-L\"of type theory
and homotopy theory that are at the heart of Homotopy Type Theory~\cite{hottbook}
and Univalent Mathematics~\cite{voevodsky:library}. A cornerstone of these connections is Voevodsky's
discovery that Martin-L\"of type theory admits a model in the category of simplicial sets~\cite{voevodsky-simplicial-model} in which identity types are interpreted as path spaces, \cf~\cite{awodey-warren:homotopy-idtype}. The definition of this model suggested a new approach to the formalisation of mathematics in type
theory, informed by the possibility of defining the homotopy level of type, and the formulation of the Univalence Axiom, which makes precise the idea that isomorphic structures should be considered as equal.
...
The question of whether simplicial model could be defined working constructively, \ie without
the use of the law of excluded middle or the axiom of choice, arose almost immediately
after Voevodsky's work and has been one of the leading questions in the area since then.


The aim of this paper is to provide a partial answer to this long-standing open problem.  In order to
explain our results, it is useful to recall that the main results of simplicial homotopy theory necessary 
...
it is not possible to prove constructively that
for a Kan complex $B$, the simplicial set $B^A$ is a Kan complex for every simplicial set $A$.
...
from simplicial to cubical, they also switched from ordinary fibrations, defined by a right lifting property,
...
also in simplicial sets and prove that they satisfy the Frobenius property, thus showing constructively
that algebraic Kan complexes are closed under exponentiation. However, the notion of a uniform Kan fibration
...
in simplicial sets. This situation, in which solving one problem leads to opening another problem, reminds closely of the game of `whack-a-mole', is summarised in~\cref{tab:whack}.
...
\end{tabular}
\medskip
\caption{The whack-a-mole game of models of Homotopy Type Theory.} 
\label{tab:whack} 
\end{table}



A recent breakthrough has been obtained by the second-named author in~\cite{henry2019qms}, who has
shown constructively that the category of simplicial sets admits a Quillen model structure in which the
fibrations are the Kan fibrations, building on the theory of weak model structures developed in~\cite{henry2018wms}. Crucially, the cofibrations in this constructive model structure are not
...
sets but only those with decidable degeneracies. Cofibrancy assumptions will be essential in our
development here. Another proof of the constructive existence of this
model structure was obtained in~\cite{GambinoN:anocp}, without relying on the results in~\cite{henry2018wms}. 


This paper builds on this work to obtain constructive counterparts of all the main results of simplicial
homotopy theory necessary to define the simplicial model of Martin-L\"of type theory extended with the
Univalence Axiom. In particular, our main results are the following:
\begin{itemize}
\item \cref{thm:MainPathObject}, asserting that, for a fibration with cofibrant domain $p \co A \to X$, the standard mapping path space factorisation provides a factorisation of the diagonal $\delta_p \co A \to A \times_X A$ as a trivial cofibration followed by a fibration;
...
\UU_c \to \U_c$ that classifies fibrations between cofibrant objects;
\item \cref{thm:fibrancy-of-u-and-uc}, asserting that $\UU_c$ is both fibrant and cofibrant;
\item \cref{thm:univalence-of-u-and-uc}, asserting that the universal fibration $\pi_c$ is univalent.
\end{itemize}
These contributions provide a further column to~\cref{tab:whack}, which is displayed
...
SOME COMMENTS ON THE METHODS OF PROOF. THE WEAK EQUIVALENCE EXTENSION
PROPERTY. 

\bigskip

Overall these results suggest the idea of defining a constructive version of the simplicial model
of Martin-L\"of type theory as follows. Contexts are interpreted as bifibrant simplicial sets. In particular,
...
This choice is justified by the fact that, for a bifibrant simplicial set $X$, the slice category~$\SSet_{/X}$ admits a model structure in which fibrations and cofibrations are the maps that become fibrations and cofibrations, respectively, in~$\SSet$. Then, the bifibrant objects in $\SSet_{/X}$
are exactly the fibrations $p \co A \to X$ with cofibrant domain. This interpretation can be 
contrasted with the way one can interpret type theory in simplicial sets is given in~\cref{tab:interpretations}.

\bigskip

SOME COMMENTS ON $\Pi$-TYPES AND COFIBRANCY AND $\eta$-RULE
...
{\em Type theory} & {\em Simplicial interpretation} & {\em Constructive simplicial interpretation} \\ \hline 
 $\Gamma$ context  & $X$ Kan complex & $X$  cofibrant Kan complex \\[3ex]
 $\Gamma \vdash A \co \text{type}$ & \begin{tabular}{c} $p \co A \to X$ Kan fibration  \\ with $X$ 
Kan complex  \end{tabular}  & \begin{tabular}{c} $p \co A \to X$ Kan fibration with \\  $X$ cofibrant
Kan complex, $A$ cofibrant  \end{tabular} \\[3ex] 
  $\Gamma \vdash a \co A$ &  $\begin{gathered} \xymatrix{X \ar[rr]^a \ar[dr]_{1_X}  & & A  \ar[dl]^{p} \\
 & X & }\end{gathered}$ & $\begin{gathered} \xymatrix{X \ar[rr]^a \ar[dr]_{1_X}  & & A  \ar[dl]^{p} \\
 & X & }\end{gathered}$ \\ \hline
\end{tabular}
\end{center}
\medskip
\caption{Two naive interpretations.} 
\label{tab:interpretations}
\end{table}

Both of the interpretations outlined in~\cref{tab:interpretations}, however, suffer from well-known 
coherence issues, in that a genuine model of type theory requires strictness conditions that do
not hold with these interpretations. For Voevodsky's simplicial model, this problem was addressed
already in~\cite{voevodsky-simplicial-model} making use of a technique due to Voevodsky. This 
could be applied using $\Pi$-types of fibrations, which we do not have.

Thus, our results offer only a partial solution to the open problem of defining a constructive
counterpart of Voevodsky's simplicial model of univalent foundations. However, the missing
ingredient appears to be only a suitable strictification or coherence theorem for the structure
that we produce in this paper, rather than additional work on simplicial homotopy theory. We
leave the investigation of such a coherence result to future work. 
...
EPSRC (via grant) and the US Air Force Office for Scientific Research (via grant). 
He is also grateful to Thierry Coquand, Karol Szumilo and Christian Sattler for useful
conversations. 
