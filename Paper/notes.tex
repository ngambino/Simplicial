...
model of HoTT because of well-known strictness issues, but it gives rise to one
...
is based on a category homotopically equivalent to that of simplicial sets. 
...
and to show that this comprehension category supports identity types, $\Sigma$-types, $\Pi$-types and a univalent type universe closed under various type constructors (\cref{th:main_ContextualCat}). In this 
...
mapping path spaces. The classical and the constructive comprehension categories are compared in~\cref{tab:interpretations}. 
...
\begin{tabular}{|c|c|c|}
\hline 
{\em Type theory} & {\em Classical} & {\em Constructive } \\ \hline 
...
in~\cite[Definition~18.1.2]{AczelP:notcst}, such that if $a \in \mathsf{u}$ and $S$ is a subset of  $a$ then 
$S \in \mathsf{u}$. We say that a set is small if it is an element of $\mathsf{u}$.
...
We leave as an open problem the question of whether a comprehension category as in~\cref{th:main_ContextualCat} can be split so as obtain a model of HoTT and so,]in particular,
obtain a constructive version of the simplicial model of HoTT. It should be noted that this question
