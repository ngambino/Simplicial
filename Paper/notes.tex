...
\begin{proof}The proof of \cite[Theorem~3.2.3]{joyal-tierney:simplicial-homotopy-theory} is completely constructive and sufficient to imply this results.
...
\qquad
\xymatrix{X + X \ar[rr]^{(1_X \, , i  p)} \ar@{>->}[d] & &  X \ar@{->>}[d]^{\sim} \\ \Delta[1] \times X \ar@{.>}[rru] \ar[r]& X \ar[r]^{p} & Y  }
\]
...
\begin{corollary}
For any cofibrant simplicial set $A$, the category $\SSet_{/A}$ admitst a weak model structure with cofibrations and fibrations  being those of $\SSet$.
\end{corollary}

\begin{proof}

It is clear that one has two weak factorization system on $\SSet_{/A}$ for all object $A$. So acording to  in \cite[Definition~2.1.11]{henry2018wms} we only have to show that bifibrant objects admits cylinder objects and path objects.
...
\begin{definition} \label{def:strhtpyequiv}
A map $f \co X' \rightarrow X$ in $\SSet$ is a \myemph{strong $k$-oriented homotopy equivalence}, where
$k \in \braces{0 \, , 1 }$, if there are maps $H$ and $H_X$ which exhibit $f$ as a retract of $\delta^k \times ' f$ as follows:
...
\begin{proof}
Under the assumptions, $p^*$ is a left Quillen functor. Therefore $p_* \co \SSet_{/Y} \rightarrow \SSet_{/X}$ is a right Quillen functor and so it preserve fibrant objects.
\end{proof}
