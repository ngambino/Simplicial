...
\section{A weakly universal fibration with cofibrant base}

\noten{I separated and put in this section the material that does not depend on
the definition of $\Weq$ and the proof of univalence. It makes sense for
this material to stay on its own.} 
...
\item  Let $[n] \in \Delta$, $a \in A_n$ and define $x \defeq p(a)$. Since $X$ is cofibrant, by the Eilenberg-Zilber lemma we can write it in a unique way as $x=s^*(x')$, where $s \co [n] \to [k]$ is a degeneracy and 
...
\]
By the universal property of the pullback, there is a unique cell $e \in E_n$ such that $w(e)=a$, and the image of $e$ in $\Delta[k]$ is the cell $s \co [n] \to [k]$, whose image in $X$ are both equal to $x=s^* x'$.
...
This will have not only all the properties of $\pi \co \UU \to \U$ established earlier, but also a cofibrant base.  In order to do this, let $\U_c$ be a cofibrant replacement of~$\U$,  which comes with a trivial fibration
\begin{equation}
\label{equ:ucu}
...
\item Let $p \co A \to X$ be a small fibration with $A$ and $X$ cofibrant. By part~(iii) of \cref{thm:universe-u} there is a pullback diagram of the form 
\[
\xymatrix{
A \ar[r] \ar[d]_p \drpullback & \UU \ar[d]^{\pi} \\
X \ar[r]_a & \U }
...
\caution{Both approaches are still included in this section.} 

\subsection*{First approach (\`a la Voevodsky)}
...
\begin{lemma} \label{thm:lemma-for-u} \hfill 
\begin{enumerate}[(i)]
\item The map $\mathsf{Weq}(\U) \rightarrow \U^\to$ is a fibration.
...
\leavevmode
\begin{enumerate}[(i)]
\item Let $f \co Y \to X$ be a trivial cofibration between cofibrant objects and consider the diagram
\[ 
...
gives us a weak equivalence $u$ between $B$ and $A[f]$ over~$X$. The completion of this diagram claimed by \cref{Prop:Homotopy_ext_prop} is exactly what one needs to produce the required diagonal filler. \qedhere
\end{enumerate}
\end{proof}



\begin{proposition} \label{thm:fibrancy-and-univalence-of-u} \hfill 
\begin{enumerate}[(i)]
...
\item Let $\delta \co \U \to \U \times \U$ be the diagonal map of $\U$ and consider its factorisation
...
\]
has a dotted diagonal filler $j$, since $r$ is a trivial cofibration and $t$ is a fibration 
by~\cref{thm:lemma-for-u}. The univalence of $\pi$ is the claim that $j$ is a weak equivalence. But by the 3-for-2 property applied to the diagram
\[
\xymatrix{
...
fibration, which holds by part~(ii) of~\cref{thm:lemma-for-u}. \qedhere
\end{enumerate}
\end{proof} 
...
\begin{lemma} \label{thm:lemma-for-uc} \hfill 
\begin{enumerate}[(i)]
\item The map $\mathsf{Weq}(\U_c) \rightarrow {\U_c}^\to$ is a fibration.
...
\begin{proof}  \hfill 
\begin{enumerate}[(i)] 
\item SHOULD BE PROVED IN GENERAL.
\item Observe that we have a diagram 
\[
\xymatrix@C=1.5cm{
...
\begin{theorem} \label{thm:fibrancy-and-univalence-of-uc} \hfill 
\begin{enumerate}[(i)]
...
\begin{proof} Part (i) follows from the fact that $\U_c$ is a cofibrant replacement of $\U$
and $\U$ is fibrant, as shown in part~(i) of~\cref{thm:fibrancy-and-univalence-of-u}.
Part~(ii) follows from part (ii) of~\cref{thm:lemma-for-uc} exactly in the 
same way as part~(ii) of~\cref{thm:fibrancy-and-univalence-of-u} follows from part~(ii) of~\cref{thm:lemma-for-u}. 
\end{proof}
