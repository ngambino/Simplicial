...
This paper investigates Voevodsky's model of Homotopy  Type Theory (HoTT), \ie Martin-L\"of type theory extended with the univalence axiom~\cite{hottbook}, in the category of simplicial sets~\cite{voevodsky-simplicial-model}. This model is of fundamental importance since it informs a new approach to the development of mathematics, guided by the notion of the homotopy level of a type and by the univalence axiom, which makes precise the idea that isomorphic structures should be considered as equal. 

The original definition of the simplicial model  was carried out working within classical Zermelo-Fraenkel set theory extended with the axiom of choice (ZFC) and two inaccessible cardinals. Here,~ZFC was used  to prove the results of simplicial homotopy theory necessary to set up the interpretation, one inaccessible cardinal  to interpret a type-theoretic universe and the other inaccessible cardinal to address strictness issues of the interepretation. Given the wide gap in proof-theoretic strength between Martin-L\"of type theory and ZFC~\cite{GrifforE:strsml} and the fact that the former is a constructive theory while the latter is classical,
...
explain our results, let us recall from~\cite{voevodsky-simplicial-model} that the main results of simplicial homotopy theory necessary 
...
model structure on the category of simplicial sets whose fibrations are the Kan fibrations, 
 the fact that pushforward along fibrations preserves fibrations 
 the existence of a  fibration $\pi \co \UU \to \U$ that classifies small fibrations, 
the fibrancy of $\U$ and, finally, the univalence of the fibration $\pi$.
...
chosen fillers for lifting problems and is required to satisfy some naturality conditions. While they
succeeded in defining constructively models of HoTT in cubical sets, none of the
categories of cubical sets considered in this work is known to be Quillen equivalent to simplicial sets or topological spaces.

As shown in~\cite{gambino2017frobenius}, it is possible to develop  a constructive theory of uniform fibrations
also in simplicial sets and prove that pushforward along a uniform fibration preserves
uniform fibrations, showing in particular that that for a simplicial set $A$ and an 
 algebraic Kan complex $B$, $B^A$ is an algebraic Kan complex. However,  uniform fibrations
are not as well-behaved in simplicial sets as in cubical sets,
since they do not admit a universal small uniform fibration, essentially because 
representables  in simplicial sets are not closed under under products while they are in cubical sets~\cite{SattlerC:faiaut}. The situation concerning these models is summarised in~\cref{tab:whack}.
...
he has shown constructively that the category of simplicial sets admits a Quillen model structure in which the
fibrations are the Kan fibrations.  Crucially, the cofibrations in this constructive model structure are not
all the monomorphisms, but rather the monomorphisms $i \co A \to B$ that are levelwise complemented 
and for which the degeneracy of $n$-simplices is decidable in $B_n \setminus A_n$ for every $[n] \in \Delta$. In particular, the cofibrant objects are in this model structure are not all the simplicial sets, but only those with decidable degeneracies. However, this model structure coincides with
the standard one as soon as the law of excluded middle is assumed. Another 
proof of the  existence of this constructive
model structure is obtained in~\cite{GambinoN:anocp} without relying on the results in~\cite{henry2018wms}. 


This paper extends these works to obtain constructive counterparts of all the other main results of simplicial
homotopy theory necessary to define the simplicial model of HoTT. In particular, our main results are the following:
\begin{itemize}
...
\item \cref{cor:Pi_types_are_fibrant}, asserting that pushforward along fibrations with cofibrant
...
\item \cref{thm:universe-uc}, asserting the existence of a universal small fibration $\pi_c \co
\UU_c \to \U_c$ that classifies small fibrations between cofibrant objects;
\item \cref{thm:fibrancy-of-u-and-uc}, asserting that the simplicial set $\UU_c$ is a cofibrant Kan complex;
...
The proofs of these results are inspired by arguments  in the existing literature, especially those in~\cite{voevodsky-simplicial-model} and~\cite{gambino2017frobenius}, but require a systematic
and careful analysis of the cofibrancy assumptions needed to carry over these proofs in a constructive
setting. This is especially important in the case of the results
for universes, for which we are not interested in defining a small fibration that classifies small fibrations as in~\cite{voevodsky-simplicial-model}, but
rather a small fibration that classifies small fibrations between cofibrant objects. Cofibrancy issues will also play an important role in
dependent products, as discussed further below.


Our results allow us to define a new interpretation of HoTT in the category of simplicial sets. The outline
of this interpretation and its differences with the standard one are presented in~\cref{tab:interpretations}. In the new interpretation, type-theoretic
contexts are mapped into bifibrant simplicial sets.   Dependent types are interpreted as fibrations $p \co A \to X$ where $X$ is  bifibrant and $A$ is cofibrant. This choice is informed by the fact that, for a bifibrant simplicial set $X$, the slice category~$\SSet_{/X}$ admits a model structure in which fibrations and cofibrations are the maps that are fibrations and cofibrations, respectively, in~$\SSet$. Then, the bifibrant objects in $\SSet_{/X}$ are exactly the fibrations $p \co A \to X$ with cofibrant domain. A type in the empty context is then interpreted as a bifibrant simplicial set. 
...
\begin{tabular}{|c|c|c|}
\hline 
{\em Type theory} & {\em Simplicial interpretation} & {\em Constructive simplicial interpretation} \\ \hline 
...
Even with the constraints imposed by this approach, we can interpret identity types, $\Sigma$-types,
and $\Pi$-types. For identity types, it is possible to interpret them not via an arbitrary
factorisation of the diagonal as a trivial cofibration followed by a fibration~\cite{awodey-warren:homotopy-idtype}, but via mapping path spaces. Indeed, \cref{thm:MainPathObject} shows that, for a fibration $p \co A \to X$ with cofibrant domain,  the factorisation of the diagonal $\delta_p \co A \to A \times_X A$ via the mapping path space
\[
\xymatrix{
A \ar[r]  & \Path(p) \ar[r] & A \times_X A &  \, ,}
\]
is a factorisation of $\delta_p$ as a trivial cofibration followed by a fibration.  A subtle issue arises when considering the interpretation of $\Pi$-types. Indeed, for a fibration $p \co A \to X$ with cofibrant domain, the result of applying the pushforward functor to a fibration $q \co B \to A$ with cofibrant domain produces a map  $\Pi_p(q) \co \Pi_A(B) \to X$, which is a fibration by~\cref{cor:Pi_types_are_fibrant} but whose domain
is not necessarily cofibrant. In order to remedy this, we can define the interpretation of $\Pi$-types to be given by a cofibrant replacement of
$\Pi_p(q) \co \Pi_A(B) \to X$, which is now a fibration with cofibrant domain. Interestingly, this definition validates 
...
within the Logical Framework, as done in~\cite{nordstrom-petersson-smith:ml}. Furthermore, experience with formalisation of mathematics
in type theory suggests that the absence of a judgemental $\eta$-rule is not too problematic: for example, Voevodsky's univalent foundations
library~\cite{voevodsky:library}, including his proof that the univalence axiom implies the principle of function extensionality, was originally developed assuming only a propositional $\eta$-rule for $\Pi$-types.

For the interpretation of a univalent type universe, we shall proceed in two steps, using again a cofibrant replacement in a crucial way. 
First, we construct a small fibration $\pi \co \UU \to \U$ that classifies small fibrations with cofibrant fibers and prove that $\U$ is fibrant. 
...
$\pi \co \UU \to \U$ and hence $\pi_c \co \UU_c \to \U_c$ are univalent fibrations. 

We refrain from claiming that our work provides a complete solution to the problem of defining a constructive version of the simplicial model of type theory
in that the interpretation outlined above does not validate the rules of type theory for substitution strictly. These rules come in two groups:
one expressing associativity and unitality of substitution and another expressing commutation of substitution with pullbacks. While 
there are well-known techniques to address these issues~\cite{voevodsky-simplicial-model}, they do not seem to apply in our constructive context.
However, the missing ingredient appears to be only a suitable coherence or strictification result for the structure defined in this paper, rather than additional results on simplicial homotopy theory. We therefore prefer to leave the investigation of such a coherence result to future work and 
simply mention that a useful tool is the definition of a cofibrant replacement functor that commutes with pullbacks in~\cite{GambinoN:anocp}. 


\smallskip

\noindent 
\textbf{Outline of the paper.} \cref{sec:preliminaries} recalls the preliminaries necessary
for the paper. In particular, we review from~\cite{henry2019qms}  the constructive version of the 
Kan-Quillen model structure. \cref{sec:basrp} establishes some basic results on pullbacks that
we will use throughout the paper. \cref{sec:pats} shows how path spaces provide factorisations
as trivial cofibration followed by fibration. \cref{sec:Pi-types} proves the restricted Frobenius property. \cref{sec:equep}
establishes the weak equivalence extension property, which we use to prove univalence.
\cref{sec:unifbb} introduces the universal
fibration $\pi_c \co \UU_c \to \U_c$. \cref{sec:fibrancy-and-univalence} proves that $\U_c$ is bifibrant and that $\pi_c \co \UU_c \to \U_c$ is univalent. 
...
He is also grateful to Thierry Coquand, Karol Szumi{\l}o and Christian Sattler for useful
conversations. 
...
Given a map $f \co Y \to X$ in $\SSet$, to be thought of as a context morphism, we write 
\[
f^* \co \SSet_{/X} \to \SSet_{/Y}
\] 
for the associated pullback functor and denote its action on a map $p \co A \to X$ (to be thought of as a dependent type in case it is a fibration with cofibrant domain and bifibrant codomain), as
\[
\xymatrix{
A[f] \ar[r] \ar[d]_{p[f]} & A \ar[d]^{p} \\
Y \ar[r]_f & X \, .}
\]
Here, one can think of $A[f]$ as the dependent type in context $Y$ obtained via the
substitutions in $f$. Since $\SSet$ is locally cartesian closed, the pullback functor 
$f^*$ has both a  left and a right adjoint, written $f_{!} \co \SSet_{/Y} \to \SSet_{/X}$ and
$f_* \co \SSet_{/Y} \to \SSet_{/X}$, respectively. The former is defined by composition with
$f$.
...
\[
\Sigma_p \co \SSet_{/A} \to \SSet_{/X} \,  , \quad  \Pi_p \co \SSet_{/A} \to \SSet_{/X} 
\]
respectively. We call these functors the \emph{dependent sum} and \emph{dependent
product} functors, respectively. Their action  on a
map $q \co B \to A$ will be written
\[
 \Sigma_p(q) \co \Sigma_A(B) \to X \, , \quad \Pi_p(q) \co \Pi_A(B) \to X \, ,
 \]
respectively. Since  $\Sigma_p$ is defined by composition, $\Sigma_A(B) \defeq B$ and $\Sigma_p(q) = q p$.
...
\begin{remark}[Substitution] In the interpretation outlined in the introduction, we use pullback functors to interpret substitution of terms into types. Given a map $f \co Y \to X$, we can how that the pullback functor $f^* \co \SSet_{/\Gamma} \to \SSet_{/\Gamma'}$ restricts to a functor $f^* \co \BFFib_{/\Gamma}  \to \BFFib_{/\Gamma'}$. 
\end{remark} 
...
\begin{remark}[$\Sigma$-types] 
It is straightforward to define an interpretation of $\Sigma$-types so that the computation rule 
and the $\eta$-rule hold as judgemental equalities. 
...
\begin{enumerate}[$(i)$]
\item The functor $f_* \co \SSet_{/Y} \rightarrow \SSet_{/X}$ preserves trivial fibrations.
\item The counit of the adjunction $f^* \dashv f_*$ is a natural isomorphism.
\item If $g \co Z \to Y$  is cofibrant in $\SSet_{/Y}$, then $f_*(g) \co f_*(Z) \to X$  is cofibrant in~$\SSet_{/X}$.
...
Since the functor $f_*$ is the right adjoint to the pullback functor $f^*$ and trivial fibrations are the maps with the the right lifting property with respect to cofibrations, $f_*$ preserves trivial fibrations if and only if $f^*$ preserve cofibrations. But this follows by~\cref{thm:cof-pbk}.

\item \noten{To be revised}  As $f$ is a monomorphism, then the forgetful functor $f_{!} \co \SSet_{/X} \rightarrow \SSet_{/Y}$ is fully faithful and hence the unit $\eta \co 1_{\SSet_{/A}} \rightarrow f^* f_{!}$ is an isomorphism. By adjointness, the counit $\varepsilon \co f^* f_* \rightarrow 1_{\SSet_{/Y}}$ is also an isomorphism. Let $v$ be a $n$-cell in $i_*(X)$.
If the image of $v$ is in $A$ then $v$ is a cell of $X \subset i_*(X)$, in which case it is decidable whether $v$ is degenerate or not.
...
where $\eta_{A}$ is  a component of the unit of adjunction  $f^* \dashv f_*$. An application of the pullback functor $f^* \co \SSet_{/X} \to \SSet_{/Y}$ to this pullback square gives the commutative square
\[\xymatrix{
\bar{B}[f] \ar[d] \ar[r]  & B \ar[d] \\
A[f] \ar@{=}[r] &A[f] 
}\]
This is a pullback since $f^* f_* \iso 1$ by part~(ii) \cref{Lemma:ForTheExtProperty}. Hence 
...
\section{A universal fibration betwen bifibrant objects}
\label{sec:unifbb}
