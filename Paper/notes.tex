...
\]
Let  $p \co A \to \Gamma$ be a fibration with cofibrant domain. Then, the composition 
...
For $p \co A \to X$, the result of applying $\Pi_p$ to a fibration with cofibrant domain $q \co B \to A$ will be written as 
\[
\Pi(q) \co \Pi_A(B) \to X \, .
\] 
