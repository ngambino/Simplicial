...
fibrations are the Kan fibrations. Moreover this model structure is cartesian and proper.  Crucially for our goals here, the cofibrations in this constructive model structure are
...
\caution{Maybe to clarify: the problem here is not so much constructivity, but the faillure of the strict $\eta$-rules.}
...
$\Pi_f(g) \co \Pi_Y(Z) \to X$, respectively. Since  $\Sigma_f$ is defined by composition,~$\Sigma_Y(Z) \defeq Z$ and~$\Sigma_f(g) = g f $. We denote the action of $f^*$ on a fibration~$p \co A \to X$ as
\[
\xymatrix{
...
\section{Basic results on pullbacks and cofibrations}
\label{sec:basrp}
...
\begin{remark}[Substitution and $\Sigma$-types] 
...
Finally, in section $3.1$ of \cite{henry2019qms} we introduced another factorization system on the category of simplicial sets called ``degeneracy quotient/degeneracy detecting map''. It has proved very useful to get certain decidability conditions and which we will need to use again to get some important lemma in the present paper. It is the factorization system cofibrantly generated by the degeneracy map $\Delta[m] \overset{\sigma}{\rightarrow} \Delta[n]$, i.e. the map induced by surjection $[m] \rightarrow [n]$. As the degeneracy map are epimorphisms in $\SSet$ (they are split epimorphism) this factorization is actually a unique factorization system. The degeneracy detecting map are the map with the (unique) right lifting property against degeneracy map. They are exactly the simplicial morphisms $f :X \rightarrow Y$ such that for $x \in X$, $f(x)$ is degenerated if and only if $x$ is degenerated. The degeneracy quotient are the pushout of coproduct of degeneracy maps, i.e. morphisms of the form $X \rightarrow X[(x_i,\sigma_i])$ where $x_i \in X_{n-1}$ are a familly of cells  and $\sigma: [n_i] \twoheadrightarrow [m_i]$ a familly of degeneracy and $X[(x_i,\sigma_i)]$ is obtained from from $X$ by freely making $x_i$ in the image of $\sigma_i $ for each $i$. There is a couple of results of \cite{henry2019qms} about this factorization system that will be of use here:


\begin{lemma} \label{lem:decidability_lift_degen_quo}
  If $p:A \rightarrow B$ is a degeneracy quotient between finite decidable simplicial set, $X$ is a cofibrant simplicial set and $f :A \rightarrow X$ any morphisms, then it is decidable whether $f$ can be factored through $p$ or not.
\end{lemma}

This is lemma 3.1.8 in \cite{henry2019qms}. The proof is simple enough so that we can explain it in a few words: $B = A[(a_i,\sigma_i)]$ for a finite collection of cells $a_i$,  hence $f$ factors through $p$ if and only if $f(a_i) =\sigma_i^*  x_i$ for each $i$, but when $X$ is cofibrant one can decide for each $i$ if this is the case or not. As there is only a finite number of such $i$, one can decide if it is the case for all $i$ or not.


\begin{proposition} \label{prop:Degen_quotient_are_pullback_stable}
The class of degeneracy quotient is stable under pullback.
\end{proposition}

See proposition 3.1.11 in \cite{henry2019qms} for the proof. This two results together have a quite interesting corollary:

\begin{proposition} \label{prop:X^kCofibrant}
\begin{enumerate}[$(i)$]
\item[]
\item If $p:A \rightarrow B$ is a degeneracy quotient and $V$ any simplicial set then $A \times V \rightarrow B \times V$ is a degeneracy quotient.
\item If $X$ is cofibrant, and $K$ is finite and decidable, then $X^K$ is cofibrant.
\item If $A \rightarrow B$ is a degeneracy quotient of finite decidable simplicial sets and $X$ is cofibrant then the induced map $X^B \rightarrow X^A$ is a cofibration between cofibrant objects.
\end{enumerate}

\end{proposition}

\begin{proof} $(i)$ is a special case of \cref{prop:Degen_quotient_are_pullback_stable}. For $(ii)$, given any $n$-cell $\Delta[n] \rightarrow X^K$ it corresponds to a morphisms $\Delta[n] \times K \rightarrow X$. For any degeneracy map $\Delta[n] \rightarrow \Delta[m]$, the map $\Delta[n] \times K \rightarrow \Delta[m] \times K$ is a degeneracy quotient by $(i)$, as it is a map between finite decidable simplicial sets lemma \cref{lem:decidability_lift_degen_quo} implies that the question of whether a map $\Delta[n] \times K \rightarrow X$ factor through $\Delta[n] \times K \rightarrow \Delta[m] \times K$ is also decidable. But this is exactly the question of degeneratness of cells in $X^K$.

Similarly, an $n$-cell of $X^A$, corresponds to a morphism $\Delta[n] \times A \rightarrow X$, it belongs to $X^B$ if and only it can factored as $\Delta[n] \times A \rightarrow \Delta[n] \times B \rightarrow X$, which is decidable by \cref{lem:decidability_lift_degen_quo} because $\Delta[n] \times A \rightarrow \Delta[n] \times B$ is a degeneracy quotient, by $(i)$, between finite decidable simplicial sets.
\end{proof} 
...
In order to interpret the formation rule, we should consider a fixed cofibrant Kan complex~$X$ and a fibration with cofibrant domain $p \co A \to X$  and define an operation mapping a fibration with cofibrant domain $q \co B \to A$  to a new fibration with cofibrant domain~$\widetilde{\Pi}_p(q) \co \widetilde{\Pi}_A(B) \to X$. For this, we proceed
...
It is also possible to give a direct proof of this claim based on the same strategy as in \cite{gambino2017frobenius} instead of using the right properness of the model structure on simplicial sets.
...
For part (iii), Let $x: \Delta[n] \rightarrow \Pi_Y A$ be a  $k$-cell, we will show that for a given degeneracy $\sigma:[n] \rightarrow [k]$ it is decidable if $x$ is ``$\sigma$-degenerated'', i.e. if $x$ factors through $\sigma: \Delta[n] \rightarrow \Delta[k]$. As degeneracy is decidable in $X$, one can freely assume that the image of $x$ in $X$ is $\sigma$-degenerated (as if it is not the case, $x$ is not $\sigma$-degenerated) hence one has a solid diagram:

\[
\xymatrix{
\Delta[n] \ar[d]_{\sigma} \ar[r] & \Pi_Y A \ar[d] \\
\Delta[k] \ar@{.>}[ur]^? \ar[r] & X  \\
}
\]

Because of the adjunction between $\Pi$ and the pullback functor along $f$, the existence of a lift as above is equivalent to the existence of a lift in:

\[
\xymatrix{
f^* \Delta[n] \ar[d]_{f^* \sigma} \ar[r]^x & A \ar[d] \\
f^* \Delta[k] \ar@{.>}[ur]^? \ar[r] & Y  \\
}
\]

The objects $f^* \Delta[n]$ and $f^* \Delta[k]$ are decidable simplicial subset of $\Delta[n]$ and $\Delta[k]$ because $f$ is itself a levelwise complemented monomorphism, hence they are both finite decidable simplicial sets. The map $f^* \sigma$ is degeneracy quotient by \cref{prop:Degen_quotient_are_pullback_stable}), hence lemma \ref{lem:decidability_lift_degen_quo} shows that the existence of such a lift is decidable (as degeneracy quotients are epimorphisms, the existence of lift making the upper triangle commutes is equivalent to the existence of lift making the square commutes).
...
\displaystyle A[f] \coprod_B \left( B \times \Delta[1] \right) \coprod_B A[f] \ar[d] \ar[r]  & A[f] \ar[d] \\
...
\begin{definition} We say that a morphisms of simplicial sets $p \co A \to X$ has cofibrant fibers if for every $x \co \Delta[n] 
\to X$ the simplicial set~$A[x]$ given by the pullback square
\[
\xymatrix{
A[x] \ar[r] \ar[d] \drpullback & A \ar[d]^{p} \\
...
\begin{lemma} \label{lem:Cofib_fiber}
\begin{enumerate}[$(i)$]
\item[] 
\item A map $f:A \rightarrow X$ with cofibrant domain has cofibrant fibers.
\item If $X$ is cofibrant and $f:A \rightarrow X$ has cofibrant fibers then $A$ is cofibrant.
\end{enumerate}
\end{lemma} 

\begin{proof} Part~(i) follows from \cref{lem:cofibrant_fiber_product}. For part~(ii), let $[n] \in \Delta$, $a \in A_n$. Since $X$ is cofibrant, by the Eilenberg-Zilber lemma we can write $p(a) \in X$ in a unique way as $p(a) =s^*(x)$, where $s \co [n] \to [k]$ is a degeneracy and 
...
For each cell of $X$,  $x \co \Delta[n] \rightarrow X$ the pullback $A[x] \rightarrow \Delta[n]$ is a small fibration with cofibrant domain by the assumptions on $p$, which means, by the definition of $\U$ that the image of the cell $x$ in $\V$ belongs to $\U$. Hence $X \rightarrow \V$ factors in $\U$, and hence $A \rightarrow X$ is actually the pullback of $\UU \rightarrow \U$.
...
side square is also a pullback. Hence $p$ is indeed a pullback of $\UU_c \rightarrow \U_c$. Conversely, any pullback  $p \co Y \rightarrow X$ of $\UU_c \rightarrow \U_c$ is a small fibrations with cofibrant fibers, hence by part~(ii) of \ref{lem:Cofib_fiber}, if $X$ is cofibrant, $p$ is small fibration between cofibrant objects.
\end{proof} 
...
Where the map $\Delta[0] \rightarrow \Delta[1]$ can be either $\delta^0$ or $\delta^1$ depending on if $0<k$ or $k<n$. See for example the last part of the proof of theorem 3.2.3 in \cite{joyal-tierney:simplicial-homotopy-theory}.
