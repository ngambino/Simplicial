...
based on a category homotopically equivalent to that of topological spaces (even assuming classical
logic). 
...
model structure are obtained in~\cite{GambinoN:anocp}. 
 ...
 The present paper builds on this work to obtain constructive counterparts of all the other main results of 
...
contexts correspond to cofibrant objects, while dependent types correspond to fibrations $p \co A \to X$ where 
...
\noindent
\textbf{Novel aspects.} The key novelty of our approach is the use of the homotopy-theoretic notion of cofibrancy to encapsulate the logical notion of decidability of degeneracies, so as to work with it in a 
...
library~\cite{voevodsky:library}. In particular, Voevodsky's proof that the univalence axiom implies the principle of function extensionality uses the propositional, rather than judgemental, $\eta$-rule for $\Pi$-types.
...
referred to as the Beck-Chevalley conditions. While there are several results to address similar issues~\cite{clairambaultdybjer2014,HofmannM:intttl,voevodsky-simplicial-model,LumsdaineP:locuoc,ShulmanM:allths}, none of them seems to apply in our  context. 

However, our results here reduce the problem of defining a constructive version of the simplicial model of Univalent Foundations to that of proving appropriate coherence result for a particular kind of 
...
as trivial cofibration followed by fibration. \cref{sec:Pi-types} discussed the dependent product
functor, proving the restricted Frobenius property. \cref{sec:equep}
...
of a small set is again small, \ie  $\forall a \, , b \,  ( a \subseteq b \land b \in \mathsf{u} \rightarrow a \in \mathsf{u} )$.  A  discussion of what can be done without this assumption is in \cref{rem:strength}. 
...
Here and throughout the paper, a {\em finite set} means a set equipped with a bijection with the set~$\{1 \, , \ldots,  n \}$, for some $n \in \mathbb{N}$. Equality in such a set
is always decidable, which we sometimes stress by saying `finite decidable set', rather than just `finite set',
so as to distinguish  this notion from other notions of finiteness, like Kuratowski finiteness.
We say that a simplicial set $X$ is finite if $X_n$ is a finite decidable set for each $n$ and  $X$ has a finite number of non-degenerate cells in total.
...
In this section we begin to show how the  comprehension category of cofibrant simplicial
...
Part~(i) of~\cref{thm:exponentials} can be proved constructively also by following the combinatorial proof in~\cite{MayJP:simoat}, using the decidability of degeneracy in $X$ given
by the cofibrancy assumption to  avoid appealing to the law of excluded middle.
...
Let $A$ be  fibrant. 
\begin{enumerate}[(i)] 
\item The object $\Path(A)$ is fibrant.
...
$\partial_0 \, , \partial_1 \co \Path(A) \to A$ are defined as the composites of $\partial$
...
cofibrant replacement in $\SSet_{/X}$ to $\Pi_p(q) \co \Pi_A(B) \to X$ so as to obtain a map with all the desired properties. The resulting map will support a categorical counterpart of the propositional
$\eta$-rule for $\Pi$-types.

We begin by showing that, for a fibration $p \co A \rightarrow X$ with cofibrant domain, the dependent product along $p$ preserves fibrations. By adjointness, this is equivalent to showing that its left adjoint, \ie 
...
%Another proof is given in~\cite[Section~3.2]{GambinoN:anocp}, adapting the argument in~\cite{SattlerC:equepu}.
...
For part~(i), since the dependent product functor $\Pi_f$ is right adjoint to the pullback functor~$f^*$ and trivial fibrations are the maps with the right lifting property with respect to cofibrations, $\Pi_f$ preserves trivial fibrations if and only if $f^*$ preserves cofibrations. But this follows from~\cref{thm:cof-pbk}.
...
For part (iii), let $y \co  \Delta[n] \rightarrow \Pi_Y(B)$ be a  $k$-cell. We will show that for a given degeneracy $\sigma \co [n] \rightarrow [k]$ it is decidable if $y$ is ``$\sigma$-degenerated'', i.e. if $y$ factors through $\sigma \co \Delta[n] \rightarrow \Delta[k]$. As degeneracy is decidable in $X$, one can freely assume that the image of   $y$ in $X$ via $\Pi_f(q)$ is $\sigma$-degenerated (as if it is not the case, $y$ is not $\sigma$-degenerated), hence one has the solid diagram
\[
\xymatrix{
...
\]
Because of the adjunction between dependent product and the pullback along $f$, the existence of a lift as above is equivalent to the existence of a lift in the diagram
\[
\xymatrix{
...
\]
The objects $f^* \Delta[n]$ and $f^* \Delta[k]$ are decidable simplicial subset of $\Delta[n]$ and $\Delta[k]$ because $f$ is itself a levelwise complemented monomorphism, hence both of them are finite decidable simplicial sets. The map $f^* \sigma$ is degeneracy quotient (by \cref{prop:Degen_quotient_are_pullback_stable}), hence \cref{lem:decidability_lift_degen_quo} shows that the existence of such a lift is decidable (as degeneracy quotients are epimorphisms, the existence of lift making the upper triangle commutes is equivalent to the existence of lift making the square commute).
...
\]
is a weak equivalence in $\SSet_{/ Y}$. Then there exist a fibration $\bar{q} \co \bar{B} \to X$, a weak equivalence $v \co \bar{B} \to A$ in $\SSet_{/X}$ and a map $B \to \bar{B}$ such that both of the other squares in the diagram above are pullbacks. 
\end{proposition}
...
\end{proof}
 ...
 Another proof of~\cref{cor:Pi_types_are_fibrant} is obtained in~\cite[Section~3.2]{GambinoN:anocp} modifying the argument in~\cite{SattlerC:equepu}.
...
we need to consider a suitable cofibrant replacement  of~$\U$ and obtain the required fibration $\pi_c \co \UU_c \to \U_c$ via a pullback. 

\medskip

As a preliminary step, let us recall that a simplicial set $A$ is \emph{small}  if $A_n$ is a small set (in the sense specified in the Introduction) for every $[n] \in \Delta$ and that a map $p \co A \to X$ of simplicial sets is \emph{small} if for every $x \co \Delta[n] 
...
\[
\mathsf{V}_n \defeq \{ F \co {\Delta_{/[n]}}^{\op} \to \Set \} \mathrlap{,}
\]
for $[n] \in \Delta$, where $\Set$ denotes the category of small sets. More generally, if $X$ is a simplicial set then maps from $X$ to $\mathsf{V}$ corresponds to functors $F \co {\Delta_{/X}}^{\op} \to \Set$, where $\Delta_{/X}$ denotes the category of elements of $X$.
...
exploiting the decidability property of cofibrations. The lemma will be used to `rectify' the 
...
is not necessarily commutative: the map $\chi$ corresponds to a presheaf~$G$ on~$\Delta_{/B}$, the map $A \to \V$ corresponds to a presheaf $F$ on~$\Delta_{/A}$, and the 
commutativity of the given solid diagram means that the composite 
\[
\xymatrix{
{\Delta_{/A}}^{\op} \ar[r] & {\Delta_{/B}}^{\op} \ar[r]^-{G} &  \Set} 
\] 
is isomorphic to $F$. But the whole diagram commutes, as required, only if they are not just isomorphic, but equal. Since $i \co A \to B$ is a cofibration, it is levelwise decidable, hence the functor $\Delta_{/A} \to \Delta_{/B}$ is a decidable inclusion at the level of objects and fully faithful. We can then redefine $G$ to be equal to $F$ on object that are in the image of $\Delta_{/A}$ and equal to the previously defined $G$ on the other objects. There is an evident way to make this into a functor, using the natural isomorphism between $F$ and the restriction of $G$, and this gives the required commutative diagram.
\end{proof}
...
\begin{proof} Part~(i) follows from \cref{lem:cofibrant_fiber_product}. For part~(ii), let $[n] \in \Delta$, $a \in A_n$. Since $X$ is cofibrant, by the constructive version of the Eilenberg-Zilber lemma~\cite[Lemma~5.1.2]{henry2018wms}, we can write $p(a) \in X$ in a unique way as $p(a) =s^*(x)$, where $s \co [n] \to [k]$ is a degeneracy and 
...
\]
and then use that $p \co A  \to \Delta[n]$ is a fibration, with chosen lifts, which produces a chosen lift in the diagram above.

For part~(ii), if a map $v \co X \to \V$ lifts to $\U$, then the map $A \to X$ classified by $v$ is a pullback of $\UU \to \U$ and therefore it is a small fibration with cofibrant fibers. Conversely, let $p \co A \to X$ be a small fibration with cofibrant fibers. Being a small 
...
Given a map $f \co A \to B$ in $\BFFib_{/X}$, a right inverse for $f$, is a map $h \co B \to A$ in $\BFFib_{/X}$ and a homotopy $f \circ h \sim 1_B$, given by an arrow $B \to \Path_{/X}(B)$ in $\BFFib_{/X}$. Similarly, a left inverse for~$f$ is an arrow~$g \co B \to A$ together with a homotopy $g \circ f \sim 1_A$, given by an arrow~$A \to \Path_{/X}(A)$ in~$\BFFib_{/X}$.
...
in  Appendix~\ref{sec:appendix} shows that  the trivial fibration $\varepsilon_X \co \mathbb{L} X \rightarrow X$ is a small fibration for every $X$, independently of whether $X$ is small or not (see \cref{rem:strength}
for a comment on the use of a `resizing rule' in the proof of this claim and part~(i) of \cref{cor:cofibrant_smallness} for details). Hence the construction of $\Pi$-types  preserves small fibrations, even between non-small objects.
\end{proof}
...
\begin{definition}
The simplicial set $D$ is the set of fix points of the idempotent $P$ acting on $D_0$.
\end{definition}
...
\begin{lemma}
The $n$-cells of $D$ are in bijection with decidable degeneracy $n$-types. Each cofibrant simplicial set $X$ has a unique map detecting degeneracy to $D$, which is the map sending a cell to its degeneracy type. \qed
\end{lemma}
...
\[ 
\xymatrix{\partial \Delta[n]  \ar[r] \ar@{>->}[d] & \mathbb{L}^{(n-1)} X \ar[d] \\ \Delta[n]  \ar[r] & X \mathrlap{.}}  
\]
...
\newblock The constructive {K}an-{Q}uillen model structure: two new proofs.
...
\newblock {\em Theory and Applications of Categories}, Vol. 35, 2020, No. 24, pp. 875-958.
...
\newblock Available from \url{http://www2.mathematik.tu-darmstadt.de/~streicher/NOTES/lift.pdf}.
...
\bibitem[Ste19]{StenzelR:unirca}
    ...
    \newblock Available from \url{http://etheses.whiterose.ac.uk/24342}.
