...
Throughout the paper, we adopt a slight abuse of language and say that a map has a right (or left) lifting property to mean a pair consisting of a map and a function 
sending elements of the class of lifting problems to a solution. In particular, for us a Kan fibration is a pair consisting of a map and a function providing solutions for
lifting problems againts horn inclusions. Morphisms between such maps will always be taken to be mere commutative squares, with no uniformity conditions with
respect to the chosen fillers.
...
\begin{proof} See~\cite[Lemma~3.1.8]{henry2019qms}.
\end{proof} 
...
\] 
is a Grothendieck fibration by part~(ii) of~\cref{lem:cofibrant_fiber_product}. We then define $p = \mathrm{cod} \circ \chi$ and obtain that
$p$ is also a Grothendieck fibration. Since $\chi$ is the forgetful functor, it clearly preserves pullbacks.
\end{proof}
...
Here and trhoughout the paper, a {\em finite set} means a set equipped with a bijection with $\{1 \, , \ldots,  n \}$, for some $n \in \mathbb{N}$. Equality in such a set
is always decidable, which we sometimes stress by saying `finite decidable', rather than just finite. This notion extends to simplicial sets in the evident way.
...
This is useful in order to support the intuition of elements of identity types as paths, a central idea in Homotopy Type Theory~\cite{hottbook}, to ensure pullback stability
...
\] 
of a map which has been proved to be a levelwise complemented inclusion in~\cref{proposition:PathObjectCofibrant} and therefore for any cell of $\Path(p)$ one can decide if it is in $A$ or not by considering it as a cell of $\Path(A)$. Since $A$ and $\Path(p)$ are cofibrant, this shows that $A \rightarrow \Path(p)$ is a cofibration. The 3-for-2 property for weak equivalences applied to $A \rightarrow \Path(p) \rightarrow A$ show that  $A \rightarrow \Path(p)$ is moreover a weak equivalence, hence a trivial cofibration.
\end{proof}
...
$ \mathbb{L}(\Pi_A(B))$, these will satisfy weaker stability properties than those of
those satisfied by $\Pi_A(B)$ in the classical simplicial model, essentially the same 
as the weak stability properties enjoyed by $\mathsf{Id}$-types in homotopy-theoretic
models, weaker even than those considered in~\cite{CarboniA:locccec}.  
...
Define an object $U$, with an arrow $\U \to \V$ by letting, for $[n] \in \Delta$, 
\[
\U_n = \{ F \in \V_n \ | \ \pi_F \co \textstyle{\int F} \to \Delta[n] \text{ is a small fibration and 
$ \textstyle{\int F}$ is cofibrant} \} \, .
\]

Note that by ``is a fibration'' we really mean ``is equiped with a fibration structure'' i.e. it comes with choices of lifts against all maps $\Lambda^k[n] \to \Delta[n]$, in particular, the map $\U \to \V$ is not injective. $U$ is a presheaf because $V$ is a presheaf, that a pullback of a fibration (with chosen lift) is again a fibration (with chosen lift) and by \cref{lem:cofibrant_fiber_product} for the cofibrancy assumptions.
...
\item The map $\pi \co \UU \to \U$ classifies small fibrations with cofibrant fibers. More precisely, a map $v:X \to \V$ can be lifted to a map $X \to \U$ if and only if it classifies a fibration with cofibrant fibers. Moreover, choice of a lift to $\U$ corresponds (functorially) to the choice of a structure of fibration on the map classified by $v$.
...
For part~(ii), if a map $v \co X \to \V$ lift to $\U$, then the map $A \to X$ it classified is a pullback of $\UU \to \U$ so it is a small fibration with cofibrant fibers. Conversely, let $p \co A \to X$ be a small fibration with cofibrant fibers. Being a small 
...
Given $f:A \to B$ an arrow in $\BFFib_{/X}$ we now construct the object $\Iseq(f)$ in $\BFFib_{/X}$ of ``weak equivalences structure'' on $f$. We will use the definition of weak equivalence as ``arrows with both a left inverse and a right inverse'', that is ``bi-invertible maps'' as in Section 4.3 of \cite{hottbook}. However, we cannot use directly the material in \cite{hottbook} as we don't quite have a model of type theory due to the coherence problems. A categorical version of this definition, bypassing the coherence problem, can be found in \cite{ShulmanM:unifid} and has been expanded in section 1.4 of \cite{StenzelR:unirca}. However we cannot use these either because they both assumes that $\Pi$-types are given by categorical exponential, i.e. right adjoint to pullback functors. This corresponds to having both judgmental $\beta$-rule and $\eta$-rule for $\Pi$ types, which we do not have if we restrict to cofibrant objects, and they also have assumptions on the path objects that would not be satisfied without restricting to cofibrant objects. For these reasons, we need to reprove some basic facts about this definition. This being said, as this is very similar to the references cited above, we will not give as much details.
...
Using our $\Pi$-types (the non-cofibrant version at this points), we can construct fibrant objects $\Linv(f) \twoheadrightarrow X$ and $\Rinv(f) \twoheadrightarrow X$, that are respectively the types of left and right inverse of $f$. Explicitely, for an object $Y \in \SSet_{/X}$ a map $Y \to \Linv(f)$ is the same as a left inverse to the map $A \times_{X} Y \to B \times_X Y$ in $\BFFib_{/Y}$.
...
\begin{proposition} Let $f:U \to V$ be an arrow in $\BFFib_{/X}$ between two fibration with cofibrant fibers $U,V \twoheadrightarrow X$. Let $A \hookrightarrow B$ a cofibration between cofibrant objects in $\SSet_{/X}$, then a lifting problem
...
\begin{proof}
 By construction, a lift $B \to \Iseq(f)$ gives both a left and a right inverse to $U \times_X B \to V\times_X B$ which is an arrow between bifibrant objects in $\SSet_{/B}$, this implies that it is a weak equivalence, and conclude the proof of this implication.

Conversely, we assume that $p^*(f): p^*(U) \to p^*(V)$ is a weak equivalence, where $p^*$ denotes the pullback along $p$ functor $\SSet_{/X} \to \SSet_{/B}$. Note that as $U$ and $V$ are assumed to have cofibrant fibers over $X$ and $A$ and $B$ are cofibrant, all the objects $p^*U$, $i^*U$, $p^*V$ and $i^* V$ are cofibrant.

Because $\Iseq(f)$ is constructed as $\Rinv(f) \times_X \Linv(f)$, we only need to construct lifts to $\Linv(f)$ and $\Rinv(f)$ separately. We start with the lift to $\Rinv(f)$. A right inverse to $f : U \to V$ in $\SSet_{/X}$ can be described as a section to a fibration $\pi_f: U \times_V \Path_{/X}(V) \to V$, indeed in terms of generalized elements it should associate to each $v \in V$ an element $u\in U$ together with a homotopy between $f(u)$ and $v$. The construction  $f \mapsto \pi_f$ is stable under pullback in the sense that $\pi_{p^* f}$ coincide with $p^*(\pi_f)$ up to natural isomorphisms. Morever, if $f$ is an equivalence $\pi_f$ also is, and hence it is a trivial fibration, in particular $p^*(\pi_f)$ and $i^*(\pi_f)$ are trivial fibrations. Now construing a lift to $\Rinv(f)$ is the same as constructing a section of $p^*(\pi_f)$ to $B$ (which is a trivial fibration) that extent the given section of $i^*(\pi_f)$. By our cofibrancy assumption, and \cref{thm:cof-pbk}, the horizontal maps in the square:
\[
...
We now move to the lift to $\Linv(f)$. A left inverse of $f$, can be described dually as a retraction of the map $\sigma_f: U \hookrightarrow \Delta[1] \times U \coprod_U V$ in $\SSet_X$. This construction is also stable under pullback (i.e. $p^*\sigma_f \simeq \sigma_{p^* f}$), it is cofibration when $U$ and $V$ are cofibrant and is a trivial cofibration if in addition is $f$ is an equivalence, so as before, $p^*(\sigma_f)$ and $i^*(\sigma_f)$ are trivial cofibrations. So the lift we are trying to construct is equivalent to a retraction in $\SSet_{/B}$ of the right map in the square below that extend the retraction already existing in $\SSet_{/A}$ of the left map:
...
where the map on the right is a fibration, and the left arrow is a weak equivalence by $2$-out-of-$3$ because both maps $p^*(\sigma_f)$ and (the pushout of) $i^* \sigma_f$ are trivial cofibration, and it is a cofibration as it is a monomorphism and the decidability condition easily follows from the fact that the arrow in the previous square where all cofibrations.
\end{proof}



Finally, we define $\Weq(U) \twoheadrightarrow U^\to$ by applying this construction $\Iseq(f)$, to the universal arrow between small fibrations with cofibrant fibers, which lives over $\U^\to$. By the previous result we have:
...
\]
has a dotted diagonal filling if and only if the map classified by $p$ is a weak equivalence.
\end{proposition}
...
\newblock Available from the second-named author's web page, 1997.
