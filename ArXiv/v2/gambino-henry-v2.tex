...
along~$h^k_n$ is isomorphic to $q \co B \to \Lambda^k_n$. The map $b \co \Delta[n] \rightarrow \U$ classifying $\bar{q}$, if chosen carefully, gives the lift we are looking for. More precisely, by \cref{thm:rectify-via-decidability} one can chose a map $\Delta[n] \rightarrow \V$ that classifies $\bar{q}$ and extend the map $\Lambda^k[n] \to \U \to \V$ classyfing $q$ (the bottom dotted arrow in the diagram below), and finally using part~(iii) of \cref{thm:universe-u} one can lift this map to map $\Delta[n] \to \U$ that gives the lift we are looking for (the diagonal dotted arrow in the diagram below). 

\[\xymatrix{
\Lambda^k[n] \ar[r] \ar[d] & \U \ar[d] \\
\Delta[n] \ar@{.>}[r] \ar@{.>}[ur] & \V \, . \\
}\]
