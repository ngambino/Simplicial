...
\]
for $[n] \in \Delta$, where $\Set$ denotes the category of small sets. More generally, if $X$ is a simplicial set then maps from $X$ to $V$ corresponds to functors $F: \Delta/X \to \Set$, where $\Delta/X$ denotes the category of elements of $X$.
...
\begin{lemma} Given a solid diagram:
...
where both squares are pullback and $i:A \to B$ is a cofibration, there exists dotted arrows that makes the third square a pullback and the diagram commutative.
\end{lemma}

\begin{proof}
The map $K \to B$ is classified by some map $\chi:B \to \V$, this gives us two dotted arrow as in the diagram above, that makes the first square a pullback. However the diagram obtained this way is not commutative: $\chi$ corresponds to a presheaf $\mathcal{K}$ on $\Delta/B$, on the other hand the map $A \to \V$ corresponds to a presheaf $\mathcal{F}$ on $\Delta/B$. The solid diagram means that the composite $\Delta/A \to \Delta/B \overset{\mathcal{K}}{\to} \Set$ is isomorphic to $\mathcal{F}$. But the complete diagram will commute only if they are not just isomorphic but equal. As $A \to B$ is a cofibration, it is levelwise decidable, hence the functor $\Delta/A \to \Delta/B$ is a fully faithful inclusion and is a decidable inclusion at the level of objects. So we can then simply redefined $\mathcal{K}$ to be equal to $\mathcal{F}$ on object that are in the image of $\Delta/A$ and equal to the previously defined $\mathcal{K}$ on the other objects. There is an obvious way to make this into a functor by using the natural isomorphism between $\mathcal{F}$ and the restriction of $\mathcal{K}$ and this gives the commutative diagram in the lemma.
\end{proof}
...
where $i$ is a cofibration between simplicial sets, has a solution if and only if the map $K \to B$ classified by $t$ is a fibration with cofibrant fibers.
...
For part~(iii), the existence of diagonal lift implies that $t$ classifies a fibration with cofibrant fibers by part~(ii). Conversely assuming that $t$ classifies a fibration with cofibrant fibers, it follows from part~(ii) that constructing a diagonal lift is the same as chosing lifts for the map $K \to B$ classified by $t$ such that the choice of lift induced on the pullback to $A$ corresponds to these given by the map $A \to \U$ in the square. But this is easily acheived as follows, for each lifting problem of $\Lambda^k[n] \to \Delta[n]$ against $K \to B$, as $A \to B$ is a cofibration, it is decidable if the map $\Delta[n] \to B$ factors through $A$ or not. Depending on this, one chose the lift given by the fact that $K \to B$ is a fibration if $\Delta[n] \to B$ does not factor in $A$ and the one chose the one provided by the fibration structure on the pullback $K[i] \to A$ given by the map $A \to \U$.
