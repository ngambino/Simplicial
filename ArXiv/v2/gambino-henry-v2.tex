...
For part (iii), Let $y \co  \Delta[n] \rightarrow \Pi_Y(B)$ be a  $k$-cell, we will show that for a given degeneracy $\sigma \co [n] \rightarrow [k]$ it is decidable if $y$ is ``$\sigma$-degenerated'', i.e. if $y$ factors through $\sigma \co \Delta[n] \rightarrow \Delta[k]$. As degeneracy is decidable in $X$, one can freely assume that the image of via $\Pi_f(q)$ of $y$ in $X$ is $\sigma$-degenerated (as if it is not the case, $y$ is not $\sigma$-degenerated) hence one has a solid diagram:
...
There is then an evident forgetful map $\U \to \V$, which is not injective. Here, $\U$ is a presheaf because $\V$ is a presheaf, the pullback of a fibration (with chosen lifts) is again a fibration (with chosen lifts), and \cref{lem:cofibrant_fiber_product} (for the cofibrancy conditions).
...
Given $f:A \to B$ an arrow in $\BFFib_{/X}$ we now construct the object $\Iseq(f)$ in $\BFFib_{/X}$ of weak equivalences structures on $f$. We will use the definition of weak equivalence as arrows with both a left inverse and a right inverse, \ie what are called bi-invertible maps in \cite[Section 4.3]{hottbook}. However, we cannot use directly the material in \cite{hottbook} as we do not quite have a model of type theory due to the coherence problems. A categorical version of this definition, bypassing the coherence problem, can be found in \cite{ShulmanM:unifid} and has been expanded in \cite[Section 1.4]{StenzelR:unirca}. However we cannot use these either because they both assumes that $\Pi$-types are given by categorical exponential, i.e. right adjoint to pullback functors. This corresponds to having both judgmental $\beta$-rule and $\eta$-rule for $\Pi$ types, which we do not have if we restrict to cofibrant objects, and they also have assumptions on the path objects that would not be satisfied without restricting to cofibrant objects. For these reasons, we need to prove \cref{thm:black-box} explicitly. This being said, as this is very similar to the references cited above, we will not give as much details.
...
Let $f:U \to V$ be an arrow in $\BFFib_{/X}$ between two fibration with cofibrant fibers $U,V \twoheadrightarrow X$. Let $A \hookrightarrow B$ a cofibration between cofibrant objects in $\SSet_{/X}$, then a lifting problem
